...
def test_ml_hierarchy():
    ...
    restriction down the hierarchy,
        chi_K(x) = phi(x),  chi_{k-1}(x) = mR(chi_k(x)),
    show that the increments Phi_k(x) = chi_k(x) - chi_{k-1}(x) add up:
        phi(x) = sum_{k=0}^K Phi_k(x)
    (Interpreting this equation requires canonical prolongation.)
    See bottom equation on page 16 of Graeser & Kornhuber (2009).'''
    ml1 = MeshLevel1D(k=1)
    ml2 = MeshLevel1D(k=2)
    assert (ml2.m == 7)
    phi = np.array([0.0,1.0,2.0,1.0,1.0,1.0,0.0,1.0,0.0])  # in V^2, i.e. zero b.c.
