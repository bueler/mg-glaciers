...
In other words, the two FAS methods require a small $Z$ which is independent of the mesh.  In the next section we will carefully measure the computational work required for the three algorithms.  The amount of work is comparable to $mZ$ in all cases, so both FAS methods are optimal algorithms which require only $O(m)$ work.  The FAS F-cycle is the fastest.


\section{Results}  \label{sec:results}

FIXME show how to run \texttt{fas/py/fas1.py}

FIXME convergence fig
...
FIXME time fig; compare \texttt{-ngsonly}
...
FIXME discuss work units; F-cycles give textbook
