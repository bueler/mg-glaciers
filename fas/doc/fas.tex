...
\end{cline}
%$
The program depends on the widely-available NumPy library \cite{Harrisetal2020}, and it imports the program imports local modules \texttt{meshlevel.py}, \texttt{problems.py}, and \texttt{cycles.py}, all from the same directory.

The various allowed options to \texttt{fas1.py} are shown by asking for help:
\begin{cline}
$ ./fas1.py -h
\end{cline}
%$
...
One can run a small suite of software (regression) tests of \texttt{fas1.py} with \,\texttt{make test}.
...
However, if instead of a small, fixed number of V-cycles we instead try a large, fixed number of NGS sweeps, i.e.~ apply the algorithm below, then the results are disappointing.
...
As also shown in Figure \ref{fig:converge}, \textsc{ngsonly}$(10^4$) generates convergence to discretization error on meshes with $m=16,32,64,128$.  However, for finer meshes ($m=256,512$) the same number of sweeps is no longer sufficient.  On yet finer meshes using the same $Z$ would make essentially no progress while taking large run time (not shown).  The reason for this behavior is that almost all of the algebraic error (section \ref{sec:femethod}), thus the numerical error as well, is in low-frequency modes which the NGS sweeps are barely able to reduce \cite[Chapter 6]{Bueler2021}.  Thus we observe the situation which multigrid schemes were designed to address \cite{BrandtLivne2011,Briggsetal2000}.  By moving the problem to coarser grids the same smoother can efficiently-reduce the remaining error.  Indeed NGS sweeps are our only actual solver, in a sense, with a multilevel mesh infrastructure which moves the problem around so that this solver can always be making progress.
...
Now that we have verified our FE method, we consider the performance of FAS-cycle solvers.  We will compare the performance of three algorithms:
...
FIXME a bit of experimentation gives 12 Vs, 3 V(1,0)s for F, and variable sweeps for NGS as needed to get discretization error

FIXME Figure \ref{fig:optimal}; we see the FAS are both optimal

\begin{figure}
\includegraphics[width=0.7\textwidth]{figs/optimal.pdf}
\caption{Run time is optimal $O(m)$ for both V-cycles and F-cycles, but explodes for (convergent) NGS sweeps.}
\label{fig:optimal}
\end{figure}

FIXME discuss work units; table of work units; F-cycles give textbook
