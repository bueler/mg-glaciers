...
As is standard in the FE literature, let $p$ denote an arbitrary node of the mesh $\mathcal{T}$, $(x_p,y_p)$ the coordinates of that node, and $\psi_p \in \mathcal{V}^h$ the corresponding $P_1$ hat function such that $\psi_p(x_q,y_q)=\delta_{pq}$ \cite{Elmanetal2014}.  Because $\psi_p \in \mathcal{D}_+^h$, if $r \in \mathcal{K}^h$ then $r + c \psi_p \in \mathcal{K}^h$ for all $c\ge b(x_p,y_p) - r(x_p,y_p)$.

As a first step toward solving \eqref{eq:fesigpweakform} we will propose slow solvers which will become smoothers \cite{Trottenbergetal2001} in our multilevel method of the next section, namely projected nonlinear Gauss-Seidel (PNGS) and projected nonlinear Jacobi (PNJ) methods \cite{KinderlehrerStampacchia1980}.  Given a current surface elevation $s\in \mathcal{K}^h$, these smoothers sweeps through the nodes in some fixed order computing the solution of one-dimensional VI problems, as implied by \eqref{eq:fesigpweakform}, and updating $s$ in some manner.

For node $p$ let $\omega_p = b(x_p,y_p) - s(x_p,y_p)$.  The one-dimensional VI at $p$ finds $c \ge \omega_p$ such that
\begin{equation}
F^h(s+c\psi_p)[(s+\gamma \psi_p) - (s+c \psi_p)] \ge 0 \label{eq:fepointwiseviEARLY}
\end{equation}
for all $\gamma \ge \omega_p$.  If
\begin{equation}
F_p^h(c) = F^h(s+c\psi_p)[\psi_p]
\end{equation}
then \eqref{eq:fepointwiseviEARLY} is equivalent to the simplified form
\begin{equation}
(\gamma - c) F_p^h(c) \ge 0. \label{eq:fepointwisevi}
\end{equation}
In the linear, increasing case the solution of \eqref{eq:fepointwisevi} is given by the formula \cite{GraeserKornhuber2009}
\begin{equation}
c = \max\left\{-\frac{\beta}{\alpha}, \omega_p\right\} \quad (\text{\emph{assumes $F_p^h(c) = \alpha c + \beta$ is linear with }} \alpha > 0), \label{eq:fepointwisevisolutionLINEAR}
\end{equation}
but this is counterfactual; in fact $F_p^h(c)$ is strongly nonlinear.  However, supposing \eqref{eq:fepointwisevi} is well-posed and solved for $c$, the current surface elevation $s$ is then updated $s \gets s + c \psi_p$.  Observe that the updated surface elevation is also in $\mathcal{K}^h$; admissibility is preserved.  If each pointwise update is done immediately then we have the (serial) PNGS smoother, while if solutions to \eqref{eq:fepointwisevi} are computed at all nodes $p$ before any updates then we get the (parallel) PNJ smoother.

FIXME need pointwise derivative of $F^h$, i.e.~the diagonal entries of the Jacobian
