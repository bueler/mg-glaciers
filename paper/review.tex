...
However, to use $\tilde\ell^j$ on the $j$th level, in a representation $\tilde{\bm{\ell}} = \{\tilde\ell^j[\psi_q^j]\}$ as a vector in $\RR^{m_j}$, it must be prolonged.  (This is the first time we have prolonged a linear functional.)  Suppose $\ell$ is in $(\mathcal{V}^{j-1})'$ and that we want to compute its prolongation $\lambda$, a linear functional in $(\mathcal{V}^j)'$.  Recalling \eqref{eq:hatcombination} once again, we need to determine the values $\lambda[\psi_q^j]$ so that
\begin{equation}
  \sum_{q=1}^{m_j} c_{pq} \lambda[\psi_q^j] = \ell[\psi_p^{j-1}] \label{eq:dualprolongderive}
\end{equation}
for $p=1,\dots,m_{j-1}$, where $c_{pq} = \psi_p^{j-1}(x_q^j)$.  Equation \eqref{eq:dualprolongderive} is an underdetermined system of $m_{j-1}$ equations in $m_j$ unknowns, exactly solvable in different ways.  Noting that $c_{pq}=1$ if $x_q^j=x_p^{j-1}$, we may choose the other values to be zero, and this yields \emph{injection prolongation},
\begin{equation}
  (\iP\ell)[\psi_q^j] = \begin{cases} \ell[\psi_p^{j-1}], & x_q^j=x_p^{j-1}, \\
                                      0, & \text{otherwise}. \end{cases}  \label{eq:injectprolong}
\end{equation}
In other words, we keep the values of $\ell$ on the coarser-level nodes and fill-in the other finer-level nodes with zero.  Using this prolongation gives a $j$th-level residual that is not smooth, but the solution of the equation on the $j$th level (i.e.~$\tilde F^j(y^j)[v]=0$ for all $v$ in $\mathcal{K}^j$) will be relatively smooth by ellipticity.
...
\pr{mcdl-vcycle}(J,F^J,\chi^J,\id{down}=1,\id{up}=1,\id{coarse}=1)\text{:} \\+
    ...
    return $w^J$
\end{pseudo*}
Note that the vectors $\omega^j$ in the second \kw{for} loop are used to add-up (accumulate) the corrections in \eqref{eq:residuallinearlevelv}, both the corrections $y^j$ on the way down and the $\eta^j$ from the way up.  The procedure returns
    $$\omega^J = y^J + \dots + y^1 + y^0 + \eta^1 + \dots + \eta^J$$
in $\mathcal{V}^J$, and then the calling solver (\pr{mcdl-solver}) does the update $w^J \gets w^J + \omega^J$.

FIXME Figure like Figure \ref{fig:gooddecomposition} but for V-cycle

FIXME note these are really $V(\id{down},0)$ and  $V(\id{down},\id{up})$ cycles
...
FIXME model performance when $h=O(2^{-J})$ and $N=2^{J+1}-1=O(2^J)$ so $J = O(\log N)$; Table \ref{tab:performancemodels}; all constants proportional to $|\log\eps|$
