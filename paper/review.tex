...
Now consider the corresponding FE method, that is, a finite-dimensional version of the same VI.  On the $j$th level we will define an obstacle $\phi^j$ in $\mathcal{V}^j$, and a constraint set $\mathcal{K}^j = \{v \ge \phi^j\} \subset \mathcal{V}^j$, and the FE problem seeks $u^j$ in $\mathcal{K}^j$ so that a finite-dimensional VI holds:
\begin{equation}
  F^j(u^j)[v-u^j] \ge 0 \quad \text{ for all } v \text{ in } \mathcal{K}^j. \label{eq:feobstacleviresidual}
\end{equation}
However, the constructions of $\phi^j$ and $F^j$ are nontrivial in a multilevel V-cycle; we have much more to say on this.
...
\pr{pgssweep}(j,w,F,\phi)\text{:} \\+
    for $p=1,\dots,m_j$ \\+
        ...
        $w \gets w + c \psi_p^j$
\end{pseudo*}
If we apply PGS on the fine-level only, not in a multilevel method, then we would choose the obstacle $\phi^J$ to be the piecewise-linear interpolant of the continuum obstacle $\varphi$.  In this case, as proven in \cite{BrandtCryer1983}, PGS converges to the solution $u^J$ of problem \eqref{eq:feobstacleviresidual}, but slowly on fine meshes.

PGS will serve as our smoother and coarse-level solver, and we will in fact apply only one or two iterations on each level.  However, we are not yet prepared to build a multilevel method for problem \eqref{eq:feobstacleviresidual} because the construction of the obstacles $\phi^j$ and constraint sets $\mathcal{K}^j$ on each mesh level is nontrivial.  In particular, we will \emph{not} choose $\phi^j$ to be the $j$th-level interpolant of the continuum obstacle $\varphi$.  Instead we will carefully decompose a fine-level constraint across the mesh-level hierarchy.


\subsection*{Multilevel constraint decomposition}  Why is the choice of obstacles in a multilevel scheme nontrivial?  A simple explanation might start by considering a coarse-level correction in a V-cycle (section \ref{sec:subspace}).  After solving the coarse-level problem we prolong the correction onto the current level, but in the obstacle problem the updated iterate $w$ would need to be admissible, that is, above the $j$-level obstacle $\phi^j$.  However, as illustrated in Figure \ref{fig:prolongobstacle}, the prolongation of an iterate which is admissible on the $j-1$ level is \emph{not} necessarily admissible on the $j$ level if the obstacles are interpolants of a (common) continuum obstacle $\varphi$.  In the glacier context, interpolated bed topography which is below the ice surface on a coarse mesh may not be, i.e.~the coarse-level ice surface may be breached, when the fine-mesh bed topography is used.
...
To resolve this issue we might try an ad hoc fix, namely overwriting $w$ with $\max\{w,\phi^j\}$, that is, we could enforce admissibility by \emph{truncation}.  However, this may \emph{re}-introduce high frequencies, requiring additional smoother effort to remove.

How do we maintain admissibility throughout the V-cycle while also not re-introducing high-frequencies?  One way forward is the \emph{multilevel constraint decomposition} method of Tai \cite{Tai2003}, a presented in V-cycle form by \cite{GraeserKornhuber2009}.  First suppose $w^J$in $\mathcal{V}^J$ is an iterate on the finest level which is admissible in the obvious sense, namely
\begin{equation}
...
\end{equation}
which is clearly nonpositive ($w^J\le 0$).  The main idea of a defect constraint is that if we modify $w^J$ by adding $y$ in $\mathcal{V}^J$ then the result is admissible if and only if $y$ is above the defect constraint:
\begin{equation}
  w^J + y \ge \varphi^J  \qquad \iff \qquad y \ge \chi^J.  \label{eq:defectmeaning}
\end{equation}

Now the main principle for the constraint decomposition method is that
\begin{quote}
\emph{we put most of the fine-level defect constraint $\chi^J$ into the coarsest levels.}
\end{quote}
To decompose $\chi^J$ across the mesh levels we use a new \emph{monotone restriction} operator $\hat R$ from $\mathcal{V}^j$ to $\mathcal{V}^{j-1}$.  For $z$ in $\mathcal{V}^j$ one computes $\hat R z$ by maximizing the node values of $z$ over the interior of the support of each $j-1$ level hat function.  Supposing $z = \sum_q z_q \psi_q^j$, we define
\begin{equation}
...
\end{equation}
for $j=J$ down to $j=1$.  These defect constraints satisfy two key properties:
\begin{itemize}
\item $\chi^j$ is in $\mathcal{V}^j$
\item $\chi^j \le \chi^{j-1}$
\end{itemize}
(Formally one should say ``$\chi^j \le P \chi^{j-1}$'' using the canonical prolongation operator \eqref{eq:canonicalprolongation}, so that we are comparing functions in the same space.)  Now the \emph{$j$th-level obstacle} is the difference:
\begin{equation}
  \phi^j = \chi^j - \chi^{j-1} \quad \text{ for } j=0,1,\dots,J,  \label{eq:levelobstacle}
\end{equation}
where we also define $\chi^{-1}=0$ so that $\phi^0 = \chi^0$.  Note $\phi^j\le 0$.

The ``constraint'' decomposition of $\chi^J$ is a ``telescoping'' sum:
\begin{equation}
  \sum_{j=0}^J \phi^j = \chi^0 + (\chi^1 - \chi^0) + (\chi^2 - \chi^1) + \dots + (\chi^J - \chi^{J-1}) = \chi^J.  \label{eq:telescopingdecomposition}
\end{equation}
Note that the sum does \emph{not} give a decomposition of the original obstacle $\varphi^J$, and furthermore the whole decomposition depends on the fine-mesh iterate $w^J$.  An example decomposition is shown in Figure \ref{fig:gooddecomposition}; compare Figure 1 in \cite{GraeserKornhuber2009}.
...
\caption{The constraint decomposition writes a fine-mesh defect obstacle $\chi^J = \varphi^J - w^J$ as a sum \eqref{eq:telescopingdecomposition} of obstacles $\phi^j = \chi^j - \chi^{j-1}$ on each level.}
\label{fig:gooddecomposition}
\end{figure}

Now that we have obstacles $\phi^j$ we define closed, convex \emph{constraint sets}
\begin{equation}
\mathcal{K}^j = \left\{v \ge \phi^j\right\} \subseteq \mathcal{V}^j \label{eq:defineKj}
\end{equation}
for $j=0,1,\dots,J$.  Note that $\phi^J$ is not the same as $\varphi^J$, so $\mathcal{K}^J$ does not approximate $\mathcal{K}$.  Instead, as suggested by Figure \ref{fig:gooddecomposition}, if the $J$th level is a high-resolution mesh and the defect constraint $\chi^J$ is smooth then $\phi^J\approx 0$. because $\chi^J$ is well-approximated on the $J-1$ level, and thus $\mathcal{K}^J \approx \{v \ge 0\}$.  Also note that, because $\phi^j \le 0$, the zero function is always in $\mathcal{K}^j$.
...
\end{equation}
be the fine-level \emph{defect constraint set}.  The sets $\mathcal{K}^j$ decompose $\mathcal{D}$:
\begin{equation}
  \mathcal{D}^J = \mathcal{K}^0 + \mathcal{K}^1 + \dots + \mathcal{K}^J. \label{eq:constraintdecomposition}
\end{equation}
Compare equation \eqref{eq:subspacedecomposition}, wherein the mesh-level subspaces decompose the whole FE space.  The proof of \eqref{eq:constraintdecomposition} uses the telescoping sum \eqref{eq:telescopingdecomposition}.

Thus every admissible modification of the current fine-mesh iterate, i.e.~every $y$ from the FE space $\mathcal{V}^J=\mathcal{V}^h$ for which $y\ge \chi^J$, can be built by choosing functions from each of the mesh-level constraint sets $\mathcal{K}^j$.  As with \eqref{eq:subspacedecomposition}, this representation is not unique.

In the ice context (section \ref{sec:sia}) the fine-mesh obstacle $\varphi^J$ will be the bed elevation and $w^J$ will be an estimate of the ice surface elevation, so $-\chi^J$ will the ice thickness.  Figure \ref{fig:icelikedecomposition} illustrates the same constraint decomposition as in Figure \ref{fig:gooddecomposition}, but as though it is a decomposition of the ice into layers.  (Compared to Figure \ref{fig:icelike}, this bed elevation $\phi^J$ has bumpy bed topography.)  However, Figure \ref{fig:gooddecomposition} plots the same decomposition in the \emph{correct} sense, so that each defect constraint $\chi^j$ and obstacle $\phi^j$ is piecewise-linear.  Figure \ref{fig:icelikedecomposition} is merely heuristic.
