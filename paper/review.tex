...
The following in-place solver defines an outer loop which sets-up the fine-level data $\chi^J$ and $F^J$, calls the cycle, and checks for convergence.  Note that one must call the solver with an admissible initial iterate; $w=\varphi^J$ is one possibility.  Compare \pr{msc-solver} in section \ref{sec:subspace}.
...
\pr{mcdl-solver}(J,w^J,F,\varphi^J,\id{rtol}=10^{-4},\id{cyclemax}=100)\text{:} \\+
    ...
    for $s=1,\dots,\id{cyclemax}$ \\+
        ...
        $\tilde\ell^J[\cdot] := - F(w^J)[\cdot]$ \qquad\qquad\qquad\quad \ct{see \eqref{eq:rhslinearlevel}} \\
