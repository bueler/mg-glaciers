...
\subsection*{Projected Gauss-Seidel}  Now consider the corresponding FE method, a finite-dimensional version of VI \eqref{eq:obstacleviresidual}.  On the $j$th level we will define an obstacle $\phi^j$ in $\mathcal{V}^j$, a constraint set $\mathcal{K}^j = \{v \ge \phi^j\} \subset \mathcal{V}^j$, and a residual function $F^j$.  Then the FE method seeks $y^j$ in $\mathcal{K}^j$ so that
\begin{equation}
  F^j(y^j)[v-y^j] \ge 0 \quad \text{ for all } v \text{ in } \mathcal{K}^j. \label{eq:feobstacleviresidual}
\end{equation}
If we work on the fine-level only, i.e.~in a single-level method, then we would choose the obstacle $\phi^J=\varphi^J$ to be the piecewise-linear interpolant of the continuum obstacle $\varphi$, and $F^J$ as the original residual $F^J(w)[\cdot] = a(w,\cdot) - \ip{f}{\cdot}$.  However, for a multilevel method the constructions of $\phi^j$ and $F^j$ are nontrivial; we will have more to say on this.

As with the linear Poisson problem, we can immediately propose an iterative solution method for solving \eqref{eq:feobstacleviresidual}, the \emph{projected Gauss-Seidel} (PGS) algorithm.  Supposing $w$ in $\mathcal{K}^j$ is an iterate, we will modify $w$ at the $p$th node so that \eqref{eq:feobstacleviresidual} holds ``at'' that node.  Note that if $w$ is admissible then $w+c\psi_p^j$ is admissible if and only if $w_p + c \ge \phi_p$ where $\phi_p = \phi^j(x_p^j)$.  (\emph{Warning.}  This equivalence holds for our piecewise-linear elements, but not for higher-order polynomial elements.)  For clarity we first describe PGS using the constrained minimization form \eqref{eq:obstaclemin}.  We define a convex scalar function from the objective functional,
\begin{equation}
i(b) = I^j(w+b\psi_p^j),
\end{equation}
where $b$ is a real number and $F^j=\grad I^j$, and we seek its minimizer over admissible perturbations,
\begin{equation}
...
\end{equation}
Then we update $w \gets w + c\psi_p^j$ and proceed to the next point.
...
\end{equation}
for all $\tilde c\ge \phi_p-w_p$.  Inequality \eqref{eq:pgspointwisevi} is a one-dimensional VI of form $(\tilde c - c)g(c) \ge 0$ on an interval, thus $g(c)=0$ or $c$ is at the end of the interval, which is \eqref{eq:pgsformula}.
...
As proven in \cite{BrandtCryer1983}, PGS converges to the solution $y^J$ of problem \eqref{eq:feobstacleviresidual}, but slowly on fine meshes.  However, note that the residual $F(w)$ does not go to zero (in general) at convergence.  Recalling the CP formulation of the problem \eqref{eq:obstaclecp}, at convergence we have $F(w)[\psi_p^j] = 0$ where $w(x_p^j) > \phi(x_p^j)$ (inactive point) but if $w(x_p^j) = \phi(x_p^j)$ (active point) then the value $F(w)[\psi_p^j]$ is only required to be nonnegative.  Thus the correct convergence criterion for repeated PGS sweeps, or any other iterative solver, is that the norm of the \emph{CP residual}, i.e.~the vector
\begin{equation}
  (\hat \bF(w))_p = \begin{cases} F(w)[\psi_p^j], & w(x_p^j) > \phi(x_p^j) \\
                                  \min\{F(w)[\psi_p^j],0\}, & \text{otherwise} \end{cases} \label{eq:cpresidual}
\end{equation}
in $\RR^{m_j}$, should be small.

PGS will serve as our smoother and coarse-level solver, applied only one or two times on each level.  However, we are not yet prepared to build a multilevel method for problem \eqref{eq:feobstacleviresidual} because the construction of the obstacles $\phi^j$ and residuals $F^j$ on each mesh level is nontrivial.  In particular, we will \emph{not} choose $\phi^j$ to be the $j$th-level interpolant of the continuum obstacle $\varphi$.  Instead we will carefully decompose a fine-level constraint across the mesh-level hierarchy.

\subsection*{Multilevel constraint decomposition (MCD)}  Why is the choice of obstacles in a multilevel scheme nontrivial?  Consider the coarse-level correction $e^{j-1}$ in a V-cycle (section \ref{sec:subspace}, equation \eqref{eq:coarsecorrection}).  We prolong this correction onto the $j$th level, but in the obstacle problem the updated iterate ($\tilde w = w+Pe^{j-1}$) would need to be admissible, that is, above $\phi^j$.  However, as illustrated in Figure \ref{fig:prolongobstacle}, the prolongation of an iterate which is admissible on the $j-1$ level is \emph{not} necessarily admissible on the $j$ level if the obstacles are interpolants of a (common) continuum obstacle $\varphi$.  (In the glacier context, an admissible coarse-mesh ice surface may be breached by the fine-mesh bed topography.)  To resolve this issue we might try an ad hoc fix, namely overwriting $\tilde w$ with $\max\{\tilde w,\phi^j\}$, that is, we could enforce admissibility by \emph{truncation}.  However, this will \emph{re}-introduce high frequencies, requiring additional smoother effort to remove.
...
How do we maintain admissibility throughout the V-cycle while also not introducing high-frequencies?  One way forward is the \emph{multilevel constraint decomposition} (MCD) method of Tai \cite{Tai2003}, which can be reformulated as a V-cycle \cite[Algorithm 4.7]{GraeserKornhuber2009}.  First suppose $w^J$in $\mathcal{V}^J$ is an iterate on the finest level which is admissible in the original sense, namely
\begin{equation}
  w^J \ge \varphi^J \label{eq:fineadmissibleiterate}
\end{equation}
where $\varphi^J$ is the $\mathcal{V}^J$-interpolant of the continuum obstacle $\varphi$, i.e.~$\varphi^J(x_p^J)=\varphi(x_p^J)$.  We define the \emph{defect constraint} \cite{GraeserKornhuber2009} of $w^J$ to be
\begin{equation}
  \chi^J = \varphi^J - w^J,  \label{eq:defectconstraint}
\end{equation}
which is clearly nonpositive ($w^J\le 0$).  The main idea here is that if we modify $w^J$ by adding $y$ from $\mathcal{V}^J$ then the result is admissible if and only if $y$ is above the defect constraint:
\begin{equation}
  w^J + y \ge \varphi^J  \qquad \iff \qquad y \ge \chi^J.  \label{eq:defectmeaning}
\end{equation}

Now the principle of the MCD method is to \emph{put as much as possible of $\chi^J$ into the coarsest levels.}  Corrections on the coarse levels are inexpensive, and they reduce the error in low frequencies, so our method facilitates making the largest corrections there.

To decompose $\chi^J$ we use a \emph{monotone restriction} operator $\mR$ from $\mathcal{V}^j$ to $\mathcal{V}^{j-1}$.  For $z$ in $\mathcal{V}^j$ one computes $\mR z$ by maximizing the node values of $z$ over the interior of the support of each $j-1$ level hat function.  That is, supposing $z = \sum_q z_q \psi_q^j$, we define
\begin{equation}
...
\end{equation}
Observe that $\mR z \ge z$.  Note that monotone restriction $\mR$ acts on functions $\mathcal{V}^j$ while canonical restriction $R$ acts on linear functionals $(\mathcal{V}^j)'$.

We define the \emph{$j$th-level defect constraint} inductively by
\begin{equation}
  \chi^{j-1} = \mR \chi^j  \label{eq:chik}
\end{equation}
for $j=J$ down to $j=1$.  These defect constraints have two key properties: $\chi^j$ is in $\mathcal{V}^j$ and $\chi^j \le \chi^{j-1}$.  (Formally one should say ``$\chi^j \le P \chi^{j-1}$'' using the canonical prolongation operator \eqref{eq:canonicalprolongation}, so that we are comparing functions in the same space.)

Now the \emph{$j$th-level obstacle} is the difference:
\begin{equation}
  \phi^j = \chi^j - \chi^{j-1} \quad \text{ for } j=0,1,\dots,J,  \label{eq:levelobstacle}
\end{equation}
where we also define $\chi^{-1}=0$ so that $\phi^0 = \chi^0$.  Note $\phi^j\le 0$.  We have now decomposed $\chi^J$ via a ``telescoping'' sum:
\begin{equation}
  \sum_{j=0}^J \phi^j = \chi^0 + (\chi^1 - \chi^0) + (\chi^2 - \chi^1) + \dots + (\chi^J - \chi^{J-1}) = \chi^J.  \label{eq:telescopingdecomposition}
\end{equation}
This sum does \emph{not} give a decomposition of the original obstacle $\varphi^J$, and furthermore it depends on the fine-mesh iterate $w^J$.  An example decomposition is shown in Figure \ref{fig:gooddecomposition}; compare Figure 1 in \cite{GraeserKornhuber2009}.  The obstacles $\phi^j$ are the gaps between the plotted defect constraints $\chi^j$.  From the obstacles $\phi^j$ we may define the closed, convex \emph{level constraint sets}
\begin{equation}
\mathcal{K}^j = \left\{v \ge \phi^j\right\} \subseteq \mathcal{V}^j \label{eq:defineKj}
\end{equation}
for $j=0,1,\dots,J$.  Because $\phi^j \le 0$, the zero function is always in $\mathcal{K}^j$.
...
\caption{The MCD method writes a fine-mesh defect obstacle $\chi^J = \varphi^J - w^J$ as a sum \eqref{eq:telescopingdecomposition} of obstacles $\phi^j = \chi^j - \chi^{j-1}$ on each level.}
\label{fig:gooddecomposition}
\end{figure}

On the finest level, note that $\phi^J$ is not the same as $\varphi^J$, so $\mathcal{K}^J$ does not approximate $\mathcal{K}$.  Instead, as suggested by Figure \ref{fig:gooddecomposition}, if the $J$th level is a high-resolution mesh and the defect constraint $\chi^J$ is smooth then $\phi^J\approx 0$. because $\chi^J$ is well-approximated on the $J-1$ level.  Thus $\mathcal{K}^J \approx \{v \ge 0\}$.

From the defect constraint on the fine level, namely $\chi^J = \varphi^J - w^J$, let
\begin{equation}
  \mathcal{D}^J = \left\{v \ge \chi^J\right\} \subset \mathcal{V}^J
\end{equation}
be the fine-level \emph{defect constraint set}.  The sets $\mathcal{K}^j$ decompose $\mathcal{D}^J$:
\begin{equation}
  \mathcal{D}^J = \mathcal{K}^0 + \mathcal{K}^1 + \dots + \mathcal{K}^J. \label{eq:constraintdecomposition}
\end{equation}
(The proof uses the telescoping sum \eqref{eq:telescopingdecomposition}.)  Compare equation \eqref{eq:subspacedecomposition}, wherein the mesh-level subspaces $\mathcal{V}^j$ decompose the whole FE space $\mathcal{V}^j$.  A constraint decomposition \eqref{eq:constraintdecomposition} is required for the method of \cite{Tai2003}, and we have implemented it following \cite{GraeserKornhuber2009}.  Because of \eqref{eq:constraintdecomposition}, every admissible modification of the current fine-mesh iterate, i.e.~every $y$ from $\mathcal{V}^J=\mathcal{V}^h$ for which $y\ge \chi^J$, can be built by choosing a function from each of the mesh-level constraint sets $\mathcal{K}^j$, though, as with \eqref{eq:subspacedecomposition}, this representation is usually not unique.

In the ice context (section \ref{sec:sia}) the fine-mesh obstacle $\varphi^J$ will be the bed elevation and $w^J$ will be an ice surface elevation, so $-\chi^J$ will be an ice thickness.  Figure \ref{fig:icelikedecomposition} illustrates the same constraint decomposition as in Figure \ref{fig:gooddecomposition}, but pictured as a decomposition of the ice into layers.  (Compared to Figure \ref{fig:icelike}, this bed elevation $\phi^J$ has bumpy bed topography.)  However, Figure \ref{fig:gooddecomposition} plots the same decomposition in the \emph{correct} sense, such that each defect constraint $\chi^j$ and obstacle $\phi^j$ is piecewise-linear (i.e.~in $\mathcal{V}^j$).  Figure \ref{fig:icelikedecomposition} is merely heuristic.

\begin{figure}
\includegraphics[width=0.7\textwidth]{fixfigs/icedec_defect.pdf}
\caption{A heuristic-only view (Figure \ref{fig:gooddecomposition} shows the correct plot): MCD visualized as a decomposition of the ``ice'' between a fine-mesh iterate $w^J$ and the fine-mesh obstacle $\varphi^J$.}
\label{fig:icelikedecomposition}
\end{figure}

\subsection*{MCD cycles}  We can now write the weak form problem on the $j$th level in three ways.  Recall that $F(w)[v] = a(w,v) - \ip{f}{v}$ denotes the original residual \eqref{eq:residual} and $I(v) = \frac{1}{2} a(v,v) - \ip{f}{v}$ is the corresponding objective function.  Suppose $w^J$ is an admissible iterate on the fine level, i.e.~$w^J\ge \varphi^J$ where $\varphi^J$ interpolates $\varphi$ in $\mathcal{H}$.

In stating the weak forms we assume we have already descended from the fine level $J$ to some coarser level $j+1$, computing admissible corrections $y^k$ from the constraint set $\mathcal{K}^k$ on each level ($k=j+1,\dots,J$).  By the meaning of the defect constraint \eqref{eq:defectmeaning}, and using the constraint decomposition \eqref{eq:constraintdecomposition}, observe that $z = w^J+y^J+\dots+y^{j+1}$ is admissible in the original sense, i.e.~$z\ge \varphi^J$.

We seek a $j$th-level correction so that any one of the following holds:
\renewcommand{\labelenumi}{\Alph{enumi}.}
\begin{enumerate}
\item Variational inequality for $y^j$ in $\mathcal{K}^j$:
   $$F(w^J+y^J+\dots+y^{j+1}+y^j)[v - y^j] \ge 0 \qquad \text{for all } v \text{ in } \mathcal{K}^j.$$
\item Constrained minimization for $y^j$ in $\mathcal{K}^j$:
   $$y^j = \argmin_{v \text{ in } \mathcal{K}^j} I(w^J+y^J+\dots+y^{j+1}+v).$$
\item Linear variational inequality for $y^j$ in $\mathcal{K}^j$:
   $$F^j(y^j)[v - y^j] \ge 0 \qquad \text{for all } v \text{ in } \mathcal{K}^j.$$
\end{enumerate}

In C we have defined the $j$th-level residual, descending inductively using linearity:
\begin{align}
  F^j(z)[\cdot] &= F(w^J+y^J+\dots+y^{j+1}+z)[\cdot] \label{eq:residuallinearlevel} \\
                &= a(w^J+y^J+\dots+y^{j+1}+z,\cdot) - \ip{f}{\cdot} \notag \\
                &= \Big(a(w^J+y^J+\dots+y^{j+1},\cdot) - \ip{f}{\cdot}\Big) + a(z,\cdot) \notag \\
                &= F^{j+1}(y^{j+1})[\cdot] + a(z,\cdot). \notag
\end{align}
Specifically, $F^J(z)[\cdot] = F(w^J)[\cdot] + a(z,\cdot)$ on the finest level.

For the classical obstacle problem, with residual $F(w)[v] = a(w,v) - \ip{f}{v}$ and a symmetric bilinear form $a(\cdot,\cdot)$, the three weak forms are equivalent.  However, the equivalence of A and B depends on symmetry, and the equivalence of A and C depends on bilinearity, and both fail for the glacier problem (section \ref{sec:sia}).  To summarize the logic, A is the most general:
   $$\mathrm{C} \quad \stackrel{a \text{ bilinear}}{\iff\strut} \quad \mathrm{A} \quad \stackrel{F=\grad I}{\iff\strut} \quad \mathrm{B}.$$

Tai \cite{Tai2003} states the MCD method using minimization form B, but the method applies to A.  However, if $F$ is not the gradient of a coercive objective then additional structural properties of $F$ are required for well-posedness (e.g.~monotonicity \cite{Bueler2020,KinderlehrerStampacchia1980}).

We are now in position to write a multilevel solver for the classical obstacle problem.  The following pseudocode, in which \pr{MCDL} stands for ``multilevel constraint decomposition linear'', is analogous to the multilevel subspace decomposition \pr{msd-solver} in section \ref{sec:subspace}.
\begin{pseudo*}
\pr{mcdl-solver}(F,\varphi,\id{rtol}=10^{-4})\text{:} \\+
    ...
    $r_0=\|\hat\bF(w)\|$ \qquad\qquad\qquad\qquad\qquad \ct{initial CP residual norm} \\
    repeat \\+
        $\chi^J = \varphi - w$ \qquad\qquad\qquad\qquad\quad \ct{fine-level defect constraint} \\
        $F^J(y)[\cdot] = F(w)[\cdot] + a(y,\cdot)$ \\
        $y=\pr{mcdl-slash}(J,F^J,\chi^J)$ \qquad\qquad \ct{or other cycle} \\
        $w\gets w+y$ \\-
    until $\|\hat\bF(w)\| \le r_0 \id{rtol}$ \\
    return $w$
\end{pseudo*}
Recall that the CP residual $\hat\bF$ is defined in \eqref{eq:cpresidual}.

Which brings us to cycle options.  FIXME state V(1,0) cycle
\begin{pseudo*}
\pr{mcdl-slash}(j,F,\chi)\text{:} \\+
    $y=0$ \\
    if $j=0$ \\+
    ...
    $\tilde F(\tilde y)[\cdot] = F(y)[\cdot] + a(y,\cdot)$ \\
    $y^{j-1} =$ \pr{mcd-slash}(j-1,R \tilde F,\chi^{j-1}) \\
    return $y + P y^{j-1}$
\end{pseudo*}
...
$\mR$ & monotone restriction of functions, $\mathcal{V}^j \to \mathcal{V}^{j-1}$; equation \eqref{eq:monotonerestriction} \\
...
$\chi^j(x)$ & $j$th-level (monotone) restriction of defect obstacle; $\chi^J = \varphi^J - w^J$ and $\chi^{j-1} = \mR \chi^j$ \\
