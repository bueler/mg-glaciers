...
\subsection*{MCD cycles}  We now write the obstacle problem on the $j$th level in three forms.  Recall that $F(w)[v] = a(w,v) - \ip{f}{v}$ denotes the original residual \eqref{eq:residual} and $I(v) = \frac{1}{2} a(v,v) - \ip{f}{v}$ is the corresponding objective function, and we suppose $w^J$ is an admissible iterate on the fine level: $w^J\ge \varphi^J$.  In stating the weak forms below we assume that we have already descended from the fine level $J$ to some coarser level $j+1$, computing admissible corrections $y^k$ from the constraint set $\mathcal{K}^k$ on each level so far ($k=j+1,\dots,J$).  By definition of the defect constraint \eqref{eq:defectmeaning}, and using the constraint decomposition \eqref{eq:constraintdecomposition}, observe that $z = w^J+y^J+\dots+y^{j+1}$ is admissible in the original sense, i.e.~$z\ge \varphi^J$.

We seek a $j$th-level correction $y^j$ in $\mathcal{K}^j$, i.e.~$y^j\ge \phi^j$, so that any one of the following holds:
\begin{itemize}
...
In \eqref{eq:mcdvilinear} we define the $j$th-level residual $F^j$ inductively using linearity:
\begin{align}
  F^j(z)[\cdot] &= F(w^J+y^J+\dots+y^{j+1}+z)[\cdot] \label{eq:residuallinearlevelderive} \\
                &= a(w^J+y^J+\dots+y^{j+1}+z,\cdot) - \ip{f}{\cdot} \notag \\
                &= a(z,\cdot) + \Big(a(w^J+y^J+\dots+y^{j+1},\cdot) - \ip{f}{\cdot}\Big) \notag \\
                &= a(z,\cdot) + F^{j+1}(y^{j+1})[\cdot]. \notag
\end{align}
(Note $F^J(z)[\cdot] = a(z,\cdot) + F(w^J)[\cdot]$ on the finest level, so $F^J \ne F$.)  This can be written
\begin{equation}
  F^j(z)[\cdot] = a(z,\cdot) - \ell^j[\cdot]  \label{eq:residuallinearlevel}
\end{equation}
for $\ell^J[\cdot] = - F(w^J)[\cdot]$ and $\ell^j[\cdot] = - F^{j+1}(y^{j+1})[\cdot]$ if $j<J$.

For the classical obstacle problem, with (continuum) residual $F(w)[v] = a(w,v) - \ip{f}{v}$ and a symmetric bilinear form $a(\cdot,\cdot)$, the three forms \eqref{eq:mcdvi}--\eqref{eq:mcdvilinear} are equivalent.  However, the equivalence of \eqref{eq:mcdvi} and \eqref{eq:mcdminimization} depends on symmetry, and the equivalence of \eqref{eq:mcdvi} and \eqref{eq:mcdvilinear} depends on bilinearity, and both fail for the glacier problem (section \ref{sec:sia}).  To summarize the logic, \eqref{eq:mcdvi} is the most general:
...
Tai \cite{Tai2003} states the MCD method using minimization form \eqref{eq:mcdminimization}, but the method applies equally to \eqref{eq:mcdvi}.  However, if $F$ is not the gradient of a coercive objective function then additional structural properties of $F$ are required for well-posedness (e.g.~monotonicity \cite{Bueler2020,KinderlehrerStampacchia1980}).

We may now write a multilevel solver for the classical obstacle problem.  The following pseudocode, in which \pr{MCDL} stands for ``multilevel constraint decomposition linear'', is analogous to the multilevel subspace decomposition \pr{msd-solver} in section \ref{sec:subspace}.
\begin{pseudo*}
...
Initially our solver applies a ``slash'' cycle, also known as a ``V(1,0)'' cycle \cite{GraeserKornhuber2009}, as in the following pseudocode:
\begin{pseudo*}
\pr{mcdl-slash}(J,F^J,\chi^J,\id{down}=1,\id{coarse}=1)\text{:} \\+
    for $k=J$ downto $k=1$ \\+
      $\chi^{k-1} = \mR \chi^k$ \\
      $\phi^k = \chi^k - P\chi^{k-1}$ \\
      $y^k = 0$ \\
      $\text{\pr{pgssweep}}^{\text{\id{down}}}(k,y^k,F^k,\phi^k)$ \\
      $\ell^{k-1}[\cdot] := - R (F^k(y^k))[\cdot]$ \\
      $F^{k-1}(y)[\cdot] := a(y,\cdot) - \ell^{k-1}[\cdot]$ \\-
    $y^0 = 0$ \\
...
An implementation detail is that functions $F(y)[\cdot]$ need not be passed as functions in the programming-language sense.  Rather, noting that the FE method is based on a subroutine which evaluates $a(w,v)$ on any mesh, the linear functional $\ell^{j-1}$ is passed as a (row) vector from which $F^{j-1}$ is constructed as needed.  That is, the signatures in the above pseudocode could be restated as ``\pr{pgssweep}$(k,y^k,\ell^k,\phi^k)$'' and ``\pr{mcdl-slash}$(J,\ell^J,\chi^J)$''.

We should describe what has been accomplished regarding theoretical convergence, for the MCD solver versus the PGS method, for the classical obstacle problem.  Let $u^h$, in $\mathcal{V}^h = \mathcal{V}^J$ and satisfying $u^h \ge \varphi^J$, be the exact solution of the finite-dimensional VI
\begin{equation}
...
\begin{theorem} \cite[Thm.~4.6]{GraeserKornhuber2009}\,  \label{thm:pgsconvergence}  FIXME Suppose $w^{(s)}$  results from $s$ applications of \pr{mcdl-slash} on the fine level, starting with any $w^{(0)}$.  There is $\rho<1$ so that
\begin{equation}
