...
\end{equation}
That is, $u$ is the minimizer, over the constraint set $\mathcal{K}$, of the scalar, quadratic \emph{objective} functional $I$.  The proof of equivalence is standard in continuous optimization.  One computes the (Gateaux) derivative, a linear functional in $\mathcal{H}'$:
\begin{equation}
  \grad I(w)[v] = \lim_{\eps\to 0} \frac{I(w+\eps v) - I(w)}{\eps} = a(u,v) - \ip{f}{v}.  \label{eq:gradobjective}
\end{equation}
Of course $\nabla I = F$, so the VI \eqref{eq:obstacleviresidual} can again be re-stated as follows:
\begin{equation}
  \nabla I(u)[v-u] \ge 0 \quad \text{ for all } v \text{ in } \mathcal{K}. \label{eq:obstaclevigradient}
\end{equation}
Thus the solution $u$ of \eqref{eq:obstaclemin} or \eqref{eq:obstaclevigradient} sits at a location in $\mathcal{K}$, possibly on the boundary of $\mathcal{K}$, where a vector pointing further into the constraint set, namely $v-u$ for $v$ in $\mathcal{K}$, points ``uphill'' on the graph of $I$.  If we think of $\nabla I(u)$ as a vector, instead of a linear functional, we could write \eqref{eq:obstaclevigradient} as $\ip{\nabla I(u)}{v-u} \ge 0$, thus the angle between $\nabla I(u)$ and $v-u$ is at most 90 degrees.

Of course, $\mathcal{K}$ in \eqref{eq:Kdefine} is an infinite-dimensional set.  To aid the intuition we propose the following low-dimensional, but global, mental image.  Imagine the constraint set $\mathcal{K}$ as analogous to the closed first quadrant in the plane, $\mathcal{Q} = \left\{x = (x_1,x_2)\,:\,x_i\ge 0\right\}$, and $I$ as analogous to a smooth, concave-up (i.e.~\emph{coercive} \cite{Evans2010}), objective function $\gamma(x)$ defined on the plane.  The solution of the corresponding VI, say $\hat x$ in $\mathcal{Q}$, may not be a location where $\nabla \gamma$ is zero.  However, $\gamma$ will have as low a value as possible, even if this means $\hat x$ is on the boundary of $\mathcal{Q}$.  At $\hat x$ all directions \emph{into} $\mathcal{Q}$ will increase the value of $\gamma$, as sketched by contours in Figure \ref{fig:cartoonplane}.  Note that the obstacle problem solution $u$ shown in Figure \ref{fig:icelike} is an infinite-dimensional version of the solution point $\hat x$ shown in the Figure \ref{fig:cartoonplane} cartoon view.
...
The glacier problem in the next section will be formulated both as a CP like \eqref{eq:obstaclecp} and a VI like \eqref{eq:obstaclevi}.  However, for general bed elevation functions (obstacles) the glacier problem has no constrained minimization formulation like \eqref{eq:obstaclemin}; the essential reason is the lack of a symmetry of the weak form in the general case \cite{JouvetBueler2012}.  In other words, for the glacier problem we possess a map like $\grad I$, from $\mathcal{H}$ to $\mathcal{H}'$, but not a scalar objective like $I$ itself.  We will return to this point in section \ref{sec:sia}.
...
The solution of the classical obstacle problem can be non-smooth at such a free boundary even when the data is arbitrarily smooth.  For example, smoothing the source term $f$ in \eqref{eq:icelikedetails} would give a solution nearly the same as shown in Figure \ref{fig:icelike}.  That is, one could make the source term $f$ into a smooth function by ``mollification'' \cite{Evans2010} so that the transition between positive and negative values of $f$ would be $C^\infty$.  However, the free boundary would remain, and at that free boundary the second derivative $u''$ would jump from value $+16=-f$ to value $-2=\varphi''$.  In general it is known that the obstacle problem solution for smooth $C^\infty$ data is limited to the Sobolev space $W^{2,\infty}$ \cite[section IV.6]{KinderlehrerStampacchia1980}, and not smoother.

Consider the corresponding FE method, that is, a finite-dimensional version of the same VI.  On the $j$th level we will define an obstacle $\phi^j$ in $\mathcal{V}^j$, and a constraint set $\mathcal{K}^j = \{v \ge \phi^j\} \subset \mathcal{V}^j$.  Our FE method seeks $u^j$ in $\mathcal{K}^j$ so that the finite-dimensional VI holds:
\begin{equation}
  F(u^j)[v-u^j] \ge 0 \quad \text{ for all } v \text{ in } \mathcal{K}^j. \label{eq:feobstacleviresidual}
\end{equation}
Furthermore, as with the linear Poisson problem, we can immediately propose an iterative solution method for solving \eqref{eq:feobstacleviresidual}.

Suppose we have an iterate $w$ in $\mathcal{K}^j$.  We will modify $w$ at the $p$th node so that \eqref{eq:feobstacleviresidual} holds ``at'' that node, but for clarity we pose the pointwise problem in constrained minimization form \eqref{eq:obstaclemin}.  Note that since $w$ is admissible then $w+c\psi_p^j$ is admissible if and only if $w_p + c \ge \phi_p$ where $\phi_p = \phi^j(x_p^j)$.  (\emph{Warning.}  This equivalence holds for our low-order, piecewise-linear elements, but it does not hold for higher-order polynomial elements, for example.)  Now define a convex scalar function, for real $b$, from the objective functional,
\begin{equation}
i(b) = I(w+b\psi_p^j),
\end{equation}
and seek its minimizer over admissible perturbations,
\begin{equation}
  c = \argmin_{b \ge \phi_p - w_p} \, i(b).
\end{equation}
The minimimum of $i(b)$ on the interval $\phi_p - w_p \le b < \infty$ occurs at either the critical point $i'(b)=0$ or at the left end of the interval.  Since $i'(b) = F(w)[\psi_p^j] + b a(\psi_p^j,\psi_p^j)$, the solution is
\begin{equation}
  c = \max\{-F(w)[\psi_p^j]/a(\psi_p^j,\psi_p^j), \phi_p - w_p\}.  \label{eq:pgsformula}
\end{equation}
We will follow this calculation by the update $w \gets w + c\psi_p^j$, and proceed to the next point.

The minimization view is not essential, however, and we can derive \eqref{eq:pgsformula} using only the VI form.  Given an iterate $w$ in $\mathcal{K}^j$ we seek $c$ such that $w+c\psi_p^j$ is also admissible and so that VI \eqref{eq:obstacleviresidual} holds with $v=w+\tilde c\psi_p^j$. That is, we find $c\ge \phi_p-w_p$ so that
\begin{equation}
F(w+c\psi_p^j)[(w+\tilde c\psi_p^j) - (w+c\psi_p^j)] = (\tilde c - c)F(w+c\psi_p^j)[\psi_p^j] \ge 0,
\end{equation}
for all $\tilde c\ge \phi_p-w_p$.  This one-dimensional VI also implies \eqref{eq:pgsformula}; the argument is elementary calculus.

In any case, the following \emph{projected Gauss-Seidel} (PGS) algorithm, a small modification of \pr{gssweep}, implements \eqref{eq:pgsformula}.
\begin{pseudo*}
\pr{pgssweep}(j,w,F,\phi)\text{:} \\+
    for $p=1,\dots,m_j$ \\+
        $c = \max\{-F(w)[\psi_p^j] \,\big/\, a(\psi_p^j,\psi_p^j),\, \phi^j(x_p^j)-w(x_p^j)\}$ \\
        $w \gets w + c \psi_p^j$
\end{pseudo*}
If we apply PGS on the fine-level only, i.e.~not multilevel, then we may simply choose the obstacle $\phi^J$ to be the piecewise-linear interpolant of the continuum obstacle $\varphi$.  In this case PGS solves problem \eqref{eq:feobstacleviresidual}, as proven in \cite{BrandtCryer1983}, but very slowly on fine meshes.

PGS will actually serve as our smoother and coarse-level solver, and we will apply only one or two iterations on each level.  However, we are not yet prepared to build a multilevel method for problem \eqref{eq:feobstacleviresidual} because the construction of the obstacles $\phi^j$ and constraint sets $\mathcal{K}^j$ on each mesh level is nontrivial.  In particular, we will \emph{not} choose $\phi^j$ to be the obvious on $j$th-level interpolant of the continuum obstacle $\varphi$.  Instead we will carefully decompose a fine-level constraint.


\subsection*{Multilevel constraint decomposition}  FIXME explain the issue that $w^{j-1}$ could be admissible w.r.t. coarse level but $Pw^{j-1}$ is not admissible w.r.t. fine level

\begin{figure}
\qquad \includegraphics[width=0.7\textwidth]{genfigs/prolongobstacle.pdf}
\caption{When prolonged, an admissible iterate on the coarse level may not be admissible on the fine level.}
