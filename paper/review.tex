...
\subsection{Nonlinear MCD cycles} \label{subsec:mcdn}  We now propose to apply a nonlinear form of the MCD method to the steady SIA VI problem \eqref{eq:siaVI}.  The method is a \emph{full approximation storage} (FAS) scheme, following a well-known multilevel strategy for nonlinear PDEs \cite{Briggsetal2000,Trottenbergetal2001}, and it extends the MCDL method of section \ref{sec:obstacle}.  This framework reduces implementation complexity relative to methods which track the inactive set \cite{Bueler2016,Jouvetetal2013,JouvetGraeser2013}, and it leads to robust and scalable numerical solutions.

As before, suppose our fine-level FE space is $\mathcal{V}^J \subset \mathcal{W}$, the space of continuous piecewise-linear functions with zero boundary values.  Suppose $b^J$ in $\mathcal{V}^J$ is the fine-level interpolant of the continuum bed $b$.  Fix $\eps>0$ in \eqref{eq:siaVI} and assume that our given iterate $w^J\approx s_\eps$ in $\mathcal{V}^J$ is admissible in the sense that $w^J \ge b^J$.  As in section \ref{sec:obstacle}, we define the fine-level defect constraint function and set:
    $$\chi^J = b^J - w^J, \qquad \mathcal{D}^J = \{v\ge \chi^J\} \subset \mathcal{V}^J.$$
Recall that a perturbed iterate is admissible, $w^J + z^J \ge b^J$, if and only if the perturbation $z^J$ is in $\mathcal{D}^J$.  As defined in detail in the next subsection \ref{subsec:pngs}, let $N^J$ be an FE approximation of the nonlinear operator $N_\eps$ in \eqref{eq:siaN}.  Let $\ell^J[v] = \ip{a}{v}$.  Our goal is to solve the fine-level FE version of VI \eqref{eq:siaVI} for an admissible perturbation $z^J$ in $\mathcal{D}^J$, that is,
\begin{equation}
...
In keeping with the fundamental MCD idea from section \ref{sec:obstacle}, we split the coarse-level correction $z^J$ into admissible parts on each level.  Recalling $\mR$ denotes the monotone restriction operator \eqref{eq:monotonerestriction}, for $j=1,\dots,J$ let
    $$\chi^{j-1} = \mR \chi^j, \qquad \phi^j = \chi^j - \chi^{j-1},$$
and define $\chi^{-1}=0$ so that $\phi^0=\chi^0$.  (See also Table \ref{tab:transfers}.  Note that the telescoping-sum decomposition \eqref{eq:telescopingdecomposition} holds.)  For $j=0,\dots,J$ let
...
Now suppose we have an admissible perturbation of $w^J$ which \emph{approximately} satisfies \eqref{eq:siaVIFEfinelevel}.  Specifically, suppose $y^{j+1},\dots,y^J$ in $\mathcal{K}^{j+1},\dots,\mathcal{K}^J$ have been computed, using smoothers as described in subsection \ref{subsec:pngs}, so that $z^{j+1}=y^{j+1}+\dots+y^J$ approximately solves \eqref{eq:siaVIFEfinelevel}.  We then seek the next coarser-level correction $y$ in $\mathcal{K}^j$ so that
\begin{equation}
N^{j+1}(w^J+z^{j+1}+y)[v-y] \ge \ell^{j+1}[v-y] \label{eq:siaVIFEprecoarseone}
\end{equation}
for all $v$ in $\mathcal{K}^j$.  (The source $\ell^{j+1}$ is defined later, but note $\ell^J[v] = \ip{a}{v}$.)  Subtracting $N^{j+1}(w^J+z^{j+1})$ from both sides of \eqref{eq:siaVIFEprecoarseone} reveals the residual,
\begin{equation}
N^{j+1}(w^J+z^{j+1}+y)[v-y] - N^{j+1}(w^J+z^{j+1})[v-y] \ge - \tilde F^{j+1}[v-y], \label{eq:siaVIFEprecoarsetwo}
\end{equation}
where our preliminary statement of the residual is
\begin{equation}
\tilde F^{j+1}[v] = N^{j+1}(w^J+z^{j+1})[v] - \ell^{j+1}[v].
\end{equation}
The effect of the smoothers should be to make $\tilde F^{j+1}$ smooth in the sense that values $\tilde F^{j+1}[\psi_p^{j+1}]$ are relatively small in a CP residual \eqref{eq:cpresidual} sense.

VI \eqref{eq:siaVIFEprecoarsetwo} should be accurately-representable on the $j$th level, and we want to state a version of it which is represented only using the $j$th-level basis.  To do this we define \emph{injection restriction} $\iR:\mathcal{V}^{j+1} \to \mathcal{V}^j$, which simply keeps the values of the solution at the coarse-level nodes and discards the fine-level nodes in between:
\begin{equation}
  (\iR v)_p = v_{2p}, \label{eq:injectionrestriction}
\end{equation}
for $p=1,\dots,m_j$.  Note that $\iR$ preserves admissibility relative to the interpolated bed elevation data on each level: $v\ge b^{j+1} \implies \iR v \ge b^j$.  Next we define $g^j$ to be the restricted $j$th-level approximation to the solution $s_\eps$, but before adding the correction $y$ from $\mathcal{K}^j$:
\begin{equation}
...
\end{equation}
(This is essentially the same source term as in a standard FAS scheme \cite{Trottenbergetal2001}.)  Note this definition uses canonical restriction $R$; see Table \ref{tab:transfers}.

Starting from \eqref{eq:siaVIFEprecoarsetwo}, our preliminary form of the $j$th-level correction VI is then
\begin{equation}
N^j(g^j+y)[v-y] - N^j(g^j)[v-y] \ge - R \left(N^{j+1}(g^{j+1}+y^{j+1}) - \ell^{j+1}\right)[v-y], \label{eq:siaVIFEFASearly}
\end{equation}
for all $v$ in $\mathcal{K}^j$.  Our desired form rewrites this by moving the source term \eqref{eq:siaell} to the right:
\begin{equation}
N^j(g^j+y)[v-y] \ge \ell^j[v-y]. \label{eq:siaVIFEFAS}
\end{equation}

From \eqref{eq:siag}, \eqref{eq:siaell}, and \eqref{eq:siaVIFEFAS} we can construct a V-cycle comparable to \pr{mcdl-vcycle} on page \pageref{ps:mcdl-vcycle}.  On descent we look for corrections $y^j$ from the down-admissible sets $\mathcal{K}^j$, but going upward we allow corrections $z^j$ from larger up-admissible sets $\mathcal{D}^j$ (Figure \ref{fig:mcdvcycle}).  The construction of the nonlinear operator $N^j$ is constructed in the next subsection, as is nontrivial construction of the \pr{pngs} smoother.
...
We propose to iterate \pr{mcdn-vcycle} until the norm of the CP residual has been reduced by a tolerance factor.  The following pseudocode essentially repeats \pr{mcdl-solver} on page \pageref{ps:mcdl-solver}.
...
\subsection{Convergence and performance results} \label{subsec:siaperformance}  FIXME first use Bueler profile to verify smoother only, then V-cycles on Bueler profile, then performance measurements on a random bed
