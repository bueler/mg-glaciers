...
To state the weak form corresponding to \eqref{eq:siacp} we must decide on appropriate function spaces, so we refer to a better-understood, but also nonlinear, problem.  Let $p=n+1>2$ and consider the $p$-\emph{Laplacian Poisson problem}.  The solution and test functions live in the Sobolev space
\begin{equation}
\mathcal{W} = W_0^{1,p}[0,L]  \label{eq:defineW}
\end{equation}
of functions with zero boundary values and $p$th-power integrable first derivatives.  For solution $u(x)$ and source $f(x)$, the strong and weak forms are 
\begin{equation}
-(|u'|^{p-2} u')' = f \quad \iff \quad \int_0^L |u'|^{p-2} u' v'\,dx = \int_0^L f v\,dx,  \label{eq:plaplacianpoisson}
\end{equation}
for all $v$ in $\mathcal{W}$.  The weak form \eqref{eq:plaplacianpoisson}, both as a PDE and in its obstacle problem version \cite{ChoeLewis1991}, is relatively well-understood because the calculus of variations can be applied---see \cite[section 5.3]{Ciarlet2002} and \cite{BarrettLiu1993}.  A unique solution $u$ in $\mathcal{W}$ exists as long as $f$ defines a bounded linear functional on $\mathcal{W}$, for example if $f$ is in $L^q[0,L]$ for $q$ conjugate to $p$, thus $p^{-1}+q^{-1}=1$.

Now, if $\eta_0>0$ then ``uniform $p$-Laplacian ellipticity'' applies to SIA equation \eqref{eq:sia} because the regularized thickness does not degenerate to zero.  That is, comparing to \eqref{eq:plaplacianpoisson}, the SIA equation $-(\mu |s'|^{p-2} s')'=a$ has bounded-below coefficient $\mu=\Gamma (s-b+\eta_0)^{p+1}$, i.e.~$\mu(x)\ge \Gamma \eta_0^{p+1} > 0$.  On the other hand \eqref{eq:sia} has a double nonlinearity \cite{Calvoetal2002} because the coefficient $\mu$ depends on $s$; the SIA equation has both porous medium \cite{Evans2010} and $p$-Laplacian nonlinearities.
...
\begin{equation}
N(s)[v] = \int_0^L \Gamma (s-b+\eta_0)^{p+1} |s'|^{p-2} s' v'\,dx \label{eq:siaN}
\end{equation}
