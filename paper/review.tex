...
\end{equation}
Let us write this time-step as a pseudocode which takes the current values $w$ and computes the new values $\tilde w$.  Up to the inversion of the mass matrix $B$, and choosing to compute the updated iterate in-place, we have a method similar to \pr{jacobi-sweep}:
...
\end{pseudo*}
In particular, the time step \id{deltat} apparently plays the role of \id{omega} in \pr{jacobi-sweep}.  However, the change $c$ to the value $w_p$ is not divided by the diagonal entries $a(\psi_p^j,\psi_p^j)$, as in the Jacobi iteration, so for \emph{conditional stability} \cite{Bueler2021} one must choose \id{deltat} $=O(h^2)$.
...
\pr{msc-solver}(w,a,\ell,\id{rtol}=10^{-4})\text{:} \\+
    $F_p = a(w,\psi_p^J) - \ell[\psi_p^J]$ \\
    $r_0 = \|F\|$ \\
    repeat \\+
...
\section{Multilevel constraint decomposition (MCD) for the obstacle problem} \label{sec:obstacle}
...
\pr{mcdl-solver}(J,w^J,a,\ell^J,\varphi^J,\id{rtol}=10^{-3},\id{cyclemax}=100)\text{:} \\+
...
\pr{mcdl-fcycle}(J,a,f,\varphi,\id{nicycles}=1)\text{:} \\+
    for $j=0,\dots,J$ \\+
        $\ell^j[\cdot] := \ip{f}{\cdot}$ \\
        $\varphi^j = \ct{interpolant of $\varphi$}$ \\
        if $j = 0$ \\+
           $w^0=\varphi^0$ \\-
        ...
        if $j < J$ \\+
            $w^j = \hat P w^{j-1}$ \qquad\qquad\qquad\qquad \ct{initial iterate for next level} \\--
    return $w^J$
\end{pseudo*}
...
\subsection{Nonlinear MCD cycles} \label{subsec:mcdn}  We propose to apply a nonlinear form of the MCD method to the SIA VI problem.  The method is a \emph{full approximation storage} (FAS) scheme \cite{Briggsetal2000,Trottenbergetal2001}, a well-known multilevel strategy for nonlinear PDEs.  Simultaneously the method generalizes the MCDL method of section \ref{sec:obstacle}.  This framework, which is both geometrically clear and relatively simple, leads to robust and scalable numerical solutions of the steady SIA model.

As before, suppose our fine-level FE space is $\mathcal{V}^J \subset \mathcal{W}$, the space of continuous piecewise-linear functions with zero boundary values.  Suppose $b^J$ in $\mathcal{V}^J$ is the fine-level interpolant of the continuum bed $b$, and assume that an iterate $w^J$ in $\mathcal{V}^J$ is admissible in the sense that $w^J \ge b^J$.  As in section \ref{sec:obstacle} we define the fine-level defect constraint function and set:
    $$\chi^J = b^J - w^J, \qquad \mathcal{D}^J = \{v\ge \chi^J\} \subset \mathcal{V}^J.$$
For a perturbation $z^J$ in $\mathcal{V}^J$, the perturbed iterate is admissible ($w^J + z^J \ge b^J$) if and only if $z^J$ is above the defect obstacle ($z^J \ge \chi^J$), that is, $z^J$ is in $\mathcal{D}^J$.

Let $N^J:\mathcal{D}^J \to (\mathcal{V}^J)'$ be an FE approximation of the nonlinear operator $N$ \eqref{eq:siaN}, and let $\ell^a[v] = \ip{a}{v}$.  (We define $N^J$ in detail in subsection \ref{subsec:pngs} below.)  Given the iterate $w^J$, our goal is to solve the fine-level FE version of VI \eqref{eq:siaVI} for an admissible perturbation $z^J$ in $\mathcal{D}^J$, that is,
\begin{equation}
N^J(w^J+z^J)[v-z^J] \ge \ell_a[v-z^J] \label{eq:siaVIFEfinelevel}
\end{equation}
for all $v$ in $\mathcal{D}^J$.  Relative to \eqref{eq:siaVI}, note $s^J=w^J+z^J$ is the desired surface elevation result, and thus that $(w^J+v)-s^J = v - z^J$.

Finite-dimensional nonlinear VI \eqref{eq:siaVIFEfinelevel} suffices when applying a single-level method.  However, to derive a coarse-level correction, and in keeping with the fundamental MCD idea from section \ref{sec:obstacle}, we split $z^J$ into admissible parts on each level.  As in section \ref{sec:obstacle}, we define a defect constraint $\chi^j$ and a (down-)obstacle $\phi^j$ on each level.  In fact, for $j=1,\dots,J$ let
    $$\chi^{j-1} = \mR \chi^j, \qquad \phi^j = \chi^j - \chi^{j-1},$$
and also define $\chi^{-1}=0$ so that $\phi^0=\chi^0$.  (Recall $\mR$ denotes the monotone restriction operator \eqref{eq:monotonerestriction}; see Table \ref{tab:transfers}.  Note that the telescoping-sum decomposition \eqref{eq:telescopingdecomposition} now holds.  Figure \ref{fig:gooddecomposition} again serves as a visualization, but with ``$\varphi^J$'' replaced by the fine-level bed interpolant $b^J$.)  Futhermore, for $j=0,\dots,J$ let
    $$\mathcal{D}^j = \{v \ge \chi^j\}, \qquad \mathcal{K}^j = \{v \ge \phi^j\}$$
be the up- and down-admissible sets, respectively; both are subsets of $\mathcal{V}^j$.  Note that $\mathcal{D}^j = \sum_{k=0}^j \mathcal{K}^k$ for any $j$.

Now suppose $y^J$ in $\mathcal{K}^J$ is an admissible perturbation of $w^J$ which \emph{approximately} satisfies
\begin{equation}
N^J(w^J+y^J)[v-y^J] \ge \ell_a[v-y^J] \label{eq:siaVIFEfineK}
\end{equation}
for all $v$ in $\mathcal{K}^J$.  (Because $y^J$ is from a smaller set than $z^J$, even solving \eqref{eq:siaVIFEfineK} exactly does not yield a solution of \eqref{eq:siaVIFEfinelevel}.)  In fact, suppose we have applied a fine-level smoother, as defined in subsection \ref{subsec:pngs}, so that the residual of \eqref{eq:siaVIFEfineK} is smooth.  Denote this $J$th-level residual, a linear functional in $(\mathcal{V}^J)'$, as
\begin{equation}
F^J[v] = N^J(w^J+y^J)[v] - \ell_a[v]. \label{eq:siaVIFEresidualfine}
\end{equation}
The effect of the smoother is to make $F^J$ smooth and furthermore to make the difference between $w^J+y^J$ and the desired solution $s^J=w^J+z^J$ smooth.

Next we seek a coarser-level correction $y$ in $\mathcal{K}^{J-1}$ so that
\begin{equation}
N^J(w^J+y^J+y)[v-y] \ge \ell_a[v-y] \label{eq:siaVIFEcoarseKearlyone}
\end{equation}
for all $v$ in $\mathcal{K}^{J-1}$.  Now subtract $N^J(w^J+y^J)[\cdot]$ from both sides of \eqref{eq:siaVIFEcoarseKearlyone}.  This reveals the residual on the right:
\begin{align}
N^J(w^J+y^J+y)[v-y] - N^J(w^J+y^J)[v-y] &\ge \ell_a[v-y] - N^J(w^J+y^J)[v-y] \label{eq:siaVIFEcoarseKearlytwo} \\
   &= - F^J[v-y]. \notag
\end{align}
Because the residual $F^J$ of \eqref{eq:siaVIFEfineK} is smooth, and the difference of inputs on the left, namely $y$, is smooth, inequality \eqref{eq:siaVIFEcoarseKearlytwo} should be well-represented on the $J-1$ level.

Thus we propose the following coarse-level equation, our first version of the FAS coarse-correction equation \cite{Trottenbergetal2001}:
\begin{equation}
N^{J-1}(\hat R(w^J+y^J)+y)[v-y] - N^{J-1}(\hat R(w^J+y^J))[v-y] \ge - R' F^J[v-y]. \label{eq:siaVIFEFASEARLY}
\end{equation}

FIXME new in above is a nonlinear operator $N^{J-1}$ (defined in next subsection)

We define \emph{injection restriction} $\iR:\mathcal{V}^j \to \mathcal{V}^{j-1}$ by
\begin{equation}
  (\iR v)_p = v_{2p} \label{eq:injectionrestriction}
\end{equation}

FIXME note $\iR$ preserves admissibility in the original sense: $\iR:\{v\ge b^J\} \to \{v\ge b^{J-1}\}$.  We rewrite \eqref{eq:siaVIFEFASEARLY}
\begin{equation}
N^{J-1}\left(\iR(w^J+y^J) + y\right)[v-y] \ge \ell^{J-1}[v-y] \label{eq:siaVIFEFAS}
\end{equation}
for $y$ in $\mathcal{K}^{J-1}$ and all $v$ in $\mathcal{K}^{J-1}$.  The new linear function includes the restricted residual $F^J$ and the $y$-independent term moved from the left side:
   $$\ell^{J-1}[v] = N^{J-1}\left(\iR(w^J+y^J)\right)[v] - R F^J[v].$$
where $R$ is canonical restriction.
...
\pr{mcdn-vcycle}(J,w^J,N,\ell^J,\chi^J,\id{down}=1,\id{coarse}=1,\id{up}=0)\text{:} \\+
    ...
    for $j=J$ downto $j=1$ \\+
      ...
      $F_p = N^j(g^j+y^j)[\psi_p^j] - \ell^j[\psi_p^j]$ \\
      $\ell^{j-1} = - R F$ \\
      $g^{j-1} = \iR(g^j + y^j)$ \\-
    $y^0 = 0$ \\
...
FIXME possible idea: $\mR$ to generate a coarse-level thickness coefficient that is better than in \eqref{eq:siaVIFEFAS}.  That is, on the coarse level the nonlinear operator is slightly different; it is more ``$\pp$-Laplacian elliptic'' in the sense that the thickness-dependent coefficient is nonzero on a coarse-level superset of the fine-level nonzero set.
...
\pr{pngs}(j,w,z,N,\ell,\phi,\id{omega}=1)\text{:} \\+
    \ct{check admissibility: $w\ge \phi$} \\
...
\section{MCD for evolving-geometry Glen-Stokes flow} \label{sec:evolvingstokes}
