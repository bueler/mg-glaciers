...
An additive alternative is a Jacobi-type smoother in which the vector residual is evaluated once at the beginning.  The following modifies \pr{jacobi-sweep} on page \pageref{ps:jacobi-sweep}.
...
This algorithm is pictured in Figure \ref{fig:mcdvcycle}, labeling the downward, pre-smoothed corrections as $y^j$ in $\mathcal{K}^j$ and the upward, accumulated, and post-smoothed corrections as $z^j$ in $\mathcal{D}^j$.  In agreement with Algorithm 4.7 in \cite{GraeserKornhuber2009}, the default \id{down} and \id{up} settings give a V(1,0) cycle.  However, any V(\id{down},\id{up}) cycle is allowed, and we will test V(1,0), V(0,1), and V(1,1)\footnote{Reference \cite{GraeserKornhuber2009} describes extending the V(1,0) cycle to a V(1,1) cycle by splitting the obstacles $\phi^j$, but it is not clear to the current author how this should be implemented.  In any case, the implementation given here allows larger coarse-level corrections.} cycles below. In the implementation, the functions $F^j(y)[\cdot]$ are not passed as functions in the pro\-gramming-language sense.  Rather, assuming a subroutine which evaluates $a(w,v)$ on each level, the linear functional $\ell^{j-1}$ is passed, from which $F^{j-1}$ is constructed in the smoother.
...
The following solver defines an outer loop which sets-up the fine-level data $\chi^J$ and $F^J$, calls the cycle, and checks for convergence.  Note that one must call the solver with an admissible initial iterate; $w=\varphi^J$ is one possibility.  Compare \pr{msc-solver} in section \ref{sec:subspace}.
\begin{pseudo*} \label{ps:mcdl-solver}
\pr{mcdl-solver}(w,F,\varphi,\id{rtol}=10^{-4},\id{cyclemax}=100)\text{:} \\+
    ...
    for $s=1,\dots,\id{cyclemax}$ \\+
        ...
        $\tilde\ell^J[\cdot] := - F(w)[\cdot]$ \qquad\qquad\qquad\quad \ct{see \eqref{eq:rhslinearlevel}} \\
        $F^J(y)[\cdot] := a(y,\cdot) - \tilde\ell^J[\cdot]$ \\
        $w\gets w+\pr{mcdl-vcycle}(F^J,\chi^J)$ \\
        if $\|\hat\bF(w)\| \le \id{rtol} \, r_0$ \\+
            break \\--
    return $w$
\end{pseudo*}

\subsection*{Convergence}  These pseudocodes have been implemented in Python.\footnote{At\, \href{https://github.com/bueler/mg-glaciers/}{\texttt{github.com/bueler/mg-glaciers/}} see the \texttt{py/1D/} directory and its \texttt{README.md}.}  We test \pr{mcdl-solver} on the two exact solutions $u$ shown in Figure \ref{fig:icelike}.  Note that the ``ice-like'' case has a discontinuous source $f$, while ``traditional'' has $f=0$, and both have smooth, parabolic obstacles.  Figure \ref{fig:convergence}, showing the numerical error norm $\|u^h-u\|_2$ as a function of mesh spacing $h$, demonstrates convergence for runs with \id{rtol} $=10^{-7}$.  The convergence rate for the traditional case is the best possible; $O(h^2)$ error is expected for the unconstrained problem \cite{Elmanetal2014}.  However, the ``ice-like'' rate is reduced because of the lower regularity of $f$.
...
\end{itemize}
We test these solvers on the ``icelike'' problem but with a more generic random bed, like the one shown in Figure \ref{fig:icelikedecomposition}.  In all cases we use the default \pr{pgs-sweeps} smoother and do slash cycles until convergence, i.e.~\eqref{eq:cpconvergencecriterion} with the default \id{rtol} $=10^{-4}$.
