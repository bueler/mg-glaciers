...
Algorithm \pr{gssweep} is stated so that there is no dependence on the dimension of the problem.  In particular, for any linear elliptic PDE which can be written abstractly as weak form \eqref{eq:weakpoisson}, \pr{gssweep} applies as stated with $m_j$ denoting the number of $j$th-level mesh nodes and $\psi_p^j$ the corresponding hat function.  Of course the rate of convergence of GS iterations depends on the details of the PDE, including its dimension.
