...
\pr{msc-vcycle}(w,F)\text{:} \\+
    ...
    for $k=1$ to $K$ \\+
        \pr{gssweep}(k,w,F)
\end{pseudo*}
Furthermore one can repeatedly apply the smoother on each level, which we denote using power notation as \pr{gssweep}$^j$, go up-and-down the mesh hierarchy in more complicated ways, or use coarser subspace corrections before finer; there are many possibilities \cite{Trottenbergetal2001}.

These \pr{msc} algorithms improve a fine-level iterate $w$ but they do not (generally) exactly solve the problem \eqref{eq:feweakpoisson} on the fine level.  Instead one iterates, testing for convergence using some tolerance on the (computable) residual.  For example, the following algorithm requires the residual to be decreased by a certain factor (with given default value).
\begin{pseudo*}
...
\caption{Subspace corrections can be downward-only as in \pr{msc-slash} (left) or a V-cycle as in \pr{msc-vcycle} (right).  Each dot in this three-level hierarchy ($K=2$) is an application of \pr{gssweep} on that level.}
\label{fig:msccycles}
\end{figure}
...
\begin{theorem} \cite[Theorem 3.10]{GraeserKornhuber2009}  There is $\rho<1$ so that if $w^{(s)}$ is the result of $s$ applications of \pr{msc-slash} or \pr{msc-vcycle} on the fine level, starting with any $w^{(0)}$, then
\begin{equation}
...
FIXME however, it is not really a fast solver, or even an algorithm?, because of how we represent stuff on $k$th level; if we represent on fine level then only \pr{gsweep}$(K,w,F)$ is fast $O(m_K)$; on other levels $k<K$ we no longer can evaluate $F(w)[\psi_p^k]$ in $O(1)$ time

\subsection*{V-cycle geometric multigrid}  Once the GS smoother has been applied on the finest ($K$) level, the error and the residual no longer contain much energy in the high-frequency modes for which we need the fine-level representation.  The GS smoother should then be applied on the $K-1$ level, but efficiently via a smaller representation of the problem using $K-1$ level data structures.  By going beyond the multilevel subspace correction ideas in this way, i.e.~by using coarse-level representations on a coarse-level problem, we can unlock the speed of multigrid.

To efficiently solve the problem on coarser levels requires addressing two concerns:
\renewcommand{\labelenumi}{\emph{\roman{enumi})}}
\begin{enumerate}
\item How do we represent an iterate $w$ and an residual $F(w)[v]=0$ on the $k$th level?
\item What equation do we hand-down to the $k-1$ level?
\end{enumerate}

The answer to \emph{i)} is relatively straightforward.  As already noted, a function $w(x)$ in $\mathcal{V}^k$ is represented by its nodal values $w_p=w(x_p^k)$, thus $w = \sum_p w_p \psi_p^k$, and $\bw = \{w_p\}$ is a vector in $\RR^{m_k}$.  Representing a residual, a linear functional in $(\mathcal{V}^k)'$, is just as simple.  The values $F_p = F(w)[\psi_p^k]$ form a vector $\bF=\{F_p\}$, also in $\RR^{m_k}$.  It is common to think of $\bw$ as a column vector and $\bF$ as a row vector, but this is not essential.

For \emph{ii)} we need to derive a new equation for the coarser level, one which relates the error and residual, and which loses little information if these quantities are smooth.  First, for a iterate $w$ in the original FE space $\mathcal{V}^h$, the residual definition \eqref{eq:residual} can be rewritten as the equation
\begin{equation}
...
\end{equation}
for all $v$ in $\mathcal{V}^h$, where $e=w-u^h$.  As stated above, on the fine level $\mathcal{V}^h=\mathcal{V}^K$ we represent $e$ by a vector of its nodal values $\be = \{e_p\}$ and $F(w)$ by its values $\bF=\{F_p\}$, both in $\RR^{m_K}$.

Equation \eqref{eq:errorequation} is actually equivalent to the original FE weak form \eqref{eq:feweakpoisson}, but if GS has already been applied on the finest ($K$th) level then both $e$ and the residual $F(w)$ are smoothed quantities which should have faithful representations in terms of only hat functions $\psi_p^{K-1}$, that is, not using all of the fine-level hats.  FIXME  Equation \eqref{eq:errorequation} can be \emph{approximated} by a new equation which will have a rapid solution if its quantities are represented using only the $K-1$ mesh.  That is, geometric multigrid proposes the following \emph{coarse-level equation},
\begin{equation}
  a(e^{K-1},v) = - (RF(w))[v]  \label{eq:coarsecorrection}
\end{equation}
for all $v$ in $\mathcal{V}^{K-1}$.  In \eqref{eq:coarsecorrection} we use a new operation $R$, to put the $k$th-level residual onto the $k-1$ level.  The \emph{canonical restriction operator} $R$ which creates a linear functional on the $k-1$ level from a linear functional $\ell^k$ on the $k$ level:
\begin{equation}
  (R \ell^k)[v] = \ell^k[v] \label{eq:canonicalrestriction}
\end{equation}
for all $v$ in $\mathcal{V}^{k-1}$.

  Given the solution $e^{k-1}$, the update
\begin{equation}
