...
We propose to apply a nonlinear form of the MCD method to the SIA problem.  This framework is geometrically clear and relatively simple, which should lead to more robust numerical implementations.  Our next step is to describe an FE discretization and a nonlinear smoother.
...
To construct a smoother we follow the VI derivation of the PGS smoother in the previous section, wherein one-dimensional VI \eqref{eq:pgspointwisevi} yields formula \eqref{eq:pgsformula}.  Recall that if $w$ is admissible on the $j$th level, i.e.~if $w\ge \phi^j$, then, for piecewise-linear finite elements, a point perturbation $w+c\psi_p^j$ is admissible if and only if $w_p+c \ge \phi_p$, equivalently $c \ge \phi_p - w_p$.  Thus, at the node $x_p^j$, our \emph{projected nonlinear Gauss-Seidel} (PNGS) smoother seeks a scalar $c \ge \phi_p - w_p$ which solves the following one-dimensional VI:
\begin{equation}
...
\subsection*{Nonlinear MCD cycles}  We derive an apparently-new nonlinear MCD method which generalizes the MCDL method of section \ref{sec:obstacle}.  The first step is to look at a two-level method.  The resulting method is a VI and MCD interpretation of the \emph{full approximation scheme} \cite{Briggsetal2000,Trottenbergetal2001} multilevel method for nonlinear PDEs.

Suppose $b^J$ in $\mathcal{V}^J \subset \mathcal{W}$ is the fine-level interpolant of the continuum bed $b(x)$, and assume that the iterate $w$ in $\mathcal{V}^J$ is admissible in the sense that $w^J \ge b^J$.  As in section \ref{sec:obstacle} we define the fine-level defect obstacle as
    $$\chi^J = b^J - w^J.$$
Recall that for a perturbation $y^J$ in $\mathcal{V}^J$, the perturbed iterate is admissible ($w^J + y^J \ge b^J$) if and only if the perturbation is above the defect obstacle ($y^J \ge \chi^J$).  As before we use the monotone restriction operator \eqref{eq:monotonerestriction} to define a defect constraint and down-obstacle on each level,
    $$\chi^j = \mR \chi^{j+1}, \qquad \phi^j = \chi^j - \chi^{j-1},$$
for $j=J-1,\dots,1,0$, including $\chi^{-1}=0$ so $\phi^0=\chi^0$, and so that the telescoping-sum decomposition $\chi^J = \sum_{j=0}^J \phi^j$ holds.  Figure \ref{fig:gooddecomposition} again serves as a visualization, but with ``$\varphi^J$'' in that Figure replaced here by the fine-level bed interpolant $b^J$.  Let
    $$\mathcal{D}^j = \{v \ge \chi^j\}, \qquad \mathcal{K}^j = \{v \ge \phi^j\}$$
be the up-admissilbe and down-admissible sets, respectively; both are subsets of the FE space $\mathcal{V}^j$.  Recall that $\mathcal{D}^j = \sum_{k=0}^j \mathcal{K}^k$ for all $j$ (by the same telescoping sum).

Let $\ell^a[v] = \ip{a}{v}$.  Our goal is to solve the fine-level version of VI \eqref{eq:siaVI}, that is,
\begin{equation}
N(s^J)[v-s^J] \ge \ell_a[v-s^J] \label{eq:siaVIFE}
\end{equation}
for all $v$ in the set $\{v \ge b^J\} \subset \mathcal{V}^J$.  Here $s^J$ is the exact solution to this FE weak form, the surface elevation result which we want.

Suppose $y^J$ in $\mathcal{K}^J$ is a down-admissible perturbation of $w^J$.  Substituting $s^J=w^J+y^J$ and $v=w^J+\tilde v$ into \eqref{eq:siaVIFE}, and removing the tilde, $y^J$ should satisfy
\begin{equation}
N(w^J+y^J)[v-y^J] \ge \ell_a[v-y^J] \label{eq:siaVIFEperturbationFINE}
\end{equation}
for all $v$ in $\mathcal{K}^J$.  In fact, let us suppose that the smoother has been applied and that $y^J$ satisfies \eqref{eq:siaVIFEperturbationFINE} to some accuracy.  Now we seek a coarser-level down-admissible perturbation $y$ in $\mathcal{K}^{J-1}$ so that
\begin{equation}
