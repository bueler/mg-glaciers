...
\subsection{Projected nonlinear Gauss-Seidel} \label{subsec:pngs}  To construct a smoother for VI \eqref{eq:siaVIFEfinelevel} or \eqref{eq:siaVIFEFAS} we follow the derivation of the PGS smoother in section \ref{sec:obstacle}.  Although a minimization-based derivation was possible for that classical obstacle problem, we also showed that one-dimensional VI \eqref{eq:pgspointwisevi} produced the PGS formula \eqref{eq:pgsformula}.  The smoother here will again use a one-dimensional VI for the SIA problem, but it will take a fixed number of Newton steps at each point, and we will need to respond when the SIA form degenerates.

For solving \eqref{eq:siaVIFEFAS}, $\phi^j$ is the obstacle on the $j$th level, with expansion $\phi^j = \sum \phi_q \psi_q^j$, and $\mathcal{K}^j = \{v \ge \phi^j\} \subset \mathcal{V}^j \subset \mathcal{W}$.  Note that $g^j+y^j \ge b^j$ follows if $y^j$ is in $\mathcal{K}^j$.

For single-level VI \eqref{eq:siaVIFEfinelevel}, or for \eqref{eq:siaVIFEFAS} when up-smoothing after the coarse-level correction,  in the following formulas replace $\phi^j$ with $\chi^j$, $\mathcal{K}^j$ with $\mathcal{D}^j$, and $y^j$ with $z^j$.

At the node $x_p^j$, our \emph{projected nonlinear Gauss-Seidel} (PNGS) smoother seeks a scalar $c$ such that $c \ge \phi_p - y_p$ which solves the following one-dimensional VI:
\begin{equation}
...
Now we have the following pseudocode for an SIA or $\pp$-Laplacian smoother.
...
\end{pseudo*}
As before there is a Jacobi version.
\begin{pseudo*} \label{ps:pnjacobi}
\pr{pnjacobi}(j,g^j,y^j,N^j,\ell^j,\phi^j)\text{:} \\+
    \ct{check admissibility: $y^j \ge \phi^j$} \\
    FIXME \\
    for $p=1,\dots,m_j$ \\+
        for $k=1,\dots,\id{newtonits}$ \\+
...
FIXME $\pp$-Laplacian case the same as possible

FIXME better patch-type smoother is a $k$-dimensional VI for $k\ge 1$ (but not too big)

\subsection{Convergence and performance results} \label{subsec:siaperformance}  FIXME first $\pp$-Laplacian
...
FIXME first use Bueler profile to verify smoother only; FIXME how do results depend on number of newton its in smoother; FIXME V-cycles and F-cycles on Bueler profile, then performance measurements on a random bed; FIXME regarding singularities compare to $\pp$-Laplacian performance
