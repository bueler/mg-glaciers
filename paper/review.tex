...
\subsection*{The PNGS smoother}  The discretization of SIA VI form \eqref{eq:siaVIresidual} proceeds essentially as in section \ref{sec:obstacle} for the VI form \eqref{eq:obstacleviresidual} of the classical obstacle problem.  One difference is that we will approximate the integral in the nonlinear form (operator) $N(s)[v]$, formula \eqref{eq:siaN}, using quadrature.  Recall that when we apply an MCD scheme for an obstacle problem, the coarse-level correction involves transformations of the residual and the obstacle from their continuum form to a different level-specific form.

Suppose as before that an iterate $w$ on the $j$th level is written in hat functions,
\begin{equation}
  w(x) = \sum_{q=1}^{m_j} w_q \psi_q^j(x), \label{eq:siatrialfunction}
\end{equation}
that similarly for the interpolant of the bed elevation, $b^j = \sum b_q \psi_q^j$.  (Note $b^j$ is not the obstacle, but it appears in the formula for $N$.)  Assume that $\ell^j[\cdot]$ is a linear functional with known values $\ell^j[\psi_p^j] = \ell_p$.  (It will be computed below.)  Recall that the support of $\psi_p^j$ is the interval $[x_p^j-h,x_p^j+h]$.  We will approximate $N$ in \eqref{eq:siaN} on the $j$th-level, namely the exact values
\begin{equation}
N(w)[\psi_p^j] = \int_{x_p^j-h}^{x_p^j+h} \Gamma (w-b+\eta_0)^{\pp+1} |w'|^{\pp-2} w' (\psi_p^j)'\,dx,  \label{eq:siaNlocal}
\end{equation}
using the trapezoid rule and the piecewise-linearity of the hat functions.  Define
\begin{equation}
\mu(x) = |x|^{\pp-2} x,  \label{eq:siahelperfcn}
\end{equation}
a continuously-differentiable function on $\RR$.  Also define
\begin{equation}
\tau_p(x)  = (x - b_p + \eta_0)^{\pp+1},  \label{eq:siahelperpower}
\end{equation}
a power of the regularized thickness when $x=w_p$.  Then the $j$th-level FE residual $F^j(w)[\cdot] \approx N(w)[\cdot] - \ell^j[\cdot]$ has the following values:
\begin{align}
...
To construct a smoother we suppose $\phi^j$ is the obstacle on the $j$th level, with expansion $\phi^j = \sum \phi_q \psi_q^j$.  Note $\phi^j$ is \emph{not} generally the interpolant of the bed $b(x)$, but rather it is a defect obstacle; see below.  Next we follow the logic of the VI derivation of the PGS smoother in the previous section, wherein one-dimensional VI \eqref{eq:pgspointwisevi} yields formula \eqref{eq:pgsformula}.  Recall that if $w$ is admissible on the $j$th level, i.e.~if $w\ge \phi^j$, then, for our piecewise-linear finite elements, a point perturbation $w+c\psi_p^j$ is admissible if and only if $w_p+c \ge \phi_p$, equivalently $c \ge \phi_p - w_p$.

At the node $x_p^j$ our \emph{projected nonlinear Gauss-Seidel} (PNGS) smoother seeks a scalar $c \ge \phi_p - w_p$ which solves the following one-dimensional VI: FIXME CONSIDER $w+c\eta_0\psi_p^j$ SO $c$ IS UNITLESS
\begin{equation}
  F^j(w+c\psi_p^j)[(w+\tilde c\psi_p^j) - (w+c\psi_p^j)] \ge 0,  \label{eq:pngspointwiseviEARLY}
\end{equation}
for all $\tilde c \ge \phi_p - w_p$.  By linearity we may write this as
\begin{equation}
  (\tilde c - c) g(c) \ge 0,  \label{eq:pngspointwisevi}
\end{equation}
where
\begin{equation}
  g(c) = F^j(w+c\psi_p^j)[\psi_p^j]  \label{eq:pngspointwiseresidual}
\end{equation}
is a differentiable function on $[\phi_p - w_p,+\infty)$ computed from formula \eqref{eq:siaresiduallevel} by replacing $w_p$ with $w_p+c$.

As was the situation for \eqref{eq:pgspointwisevi}, the solution of VI \eqref{eq:pngspointwisevi} is either the unique value $c$ in the admissible interval $[\phi_p - w_p,+\infty)$ such that $g(c)=0$, or it is the left endpoint of that interval.  In fact, regarding the practical matter of constructing a reliable smoother, it is important that we justify why VI \eqref{eq:pngspointwisevi} is well-posed in this sense.  FIXME It suffices to show that $g(c)$ is strictly increasing.  First observe that $\mu'(x) = (\pp-1) |x|^{\pp-2}$ is nonnegative on the real line, and that $\tau_p'(x) = (\pp+1) (x-b_p+\eta_0)^{\pp}$ is strictly positive on the admissible interval\footnote{FIXME conflict between $[\phi_p - w_p,+\infty)$ and $[b_p - w_p,+\infty)$} because $\eta_0>0$.  The derivative of $g$ is
\begin{align}
...
\end{align}
It FIXME is easily seen that $g'(c)$ is nonnegative, and FIXME, thus $g(c)$ is strictly increasing, and thus the solution of \eqref{eq:pngspointwisevi} is unique.

However, unlike the corresponding problem for the classical obstacle problem, the solution of \eqref{eq:pngspointwisevi} cannot be found exactly.  Instead we set up a Newton iteration to compute $c$.  FIXME Newton
\begin{equation}
  c = \max\left\{-\frac{F^j(w)[\psi_p^j]}{FIXME}, \, \phi_p - w_p\right\}  \label{eq:pngsformula}
\end{equation}
