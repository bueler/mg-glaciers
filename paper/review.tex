...
Initially our solver applies a ``slash'' cycle, also known as a ``V(1,0)'' cycle \cite{GraeserKornhuber2009}.  Here is a recursive pseudocode:
\begin{pseudo*}
\pr{mcdl-slash}(j,F,\chi,\id{down}=1,\id{coarse}=1)\text{:} \\+
    $y=0$ \\
    if $j=0$ \\+
    ...
    $\ell^{j-1}[\cdot] := - R (F(y))[\cdot]$ \\
    $F^{j-1}(y)[\cdot] := a(y,\cdot) - \ell^{j-1}[\cdot]$ \\
    $y^{j-1} =$ \pr{mcd-slash}(j-1,F^{j-1},\chi^{j-1}) \\
    return $y + P y^{j-1}$
\end{pseudo*}
An implementation detail is that the function $F^{j-1}(y)[\cdot]$ need not be passed as a function.  Rather, the FE method has a subroutine which evaluates $a(w,v)$ on any mesh, and then the linear functional $\ell^{j-1}$ is passed as a (row) vector from which $F^{j-1}$ is constructed.  The signatures in the above pseudocode could be stated as ``\pr{pgssweep}$(j,y,\ell,\phi)$'' and ``\pr{mcdl-slash}$(j,\ell,\chi)$''.
