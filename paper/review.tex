...
\end{equation}
Moving the source term \eqref{eq:siaell} to the right, we seek a correction $y^j$ in $\mathcal{K}^j$ so that
\begin{equation}
N^j(g^j+y^j)[v-y^j] \ge \ell^j[v-y^j] \label{eq:siaVIFEFAS}
\end{equation}
for all $v$ in $\mathcal{K}^j$.

From \eqref{eq:siag}, \eqref{eq:siaell}, and \eqref{eq:siaVIFEFAS} we can construct a V-cycle comparable to \pr{mcdl-vcycle} on page \pageref{ps:mcdl-vcycle}.  On descent we look for corrections $y^j$ from the down-admissible sets $\mathcal{K}^j$, but going upward we allow corrections $z^j$ from larger up-admissible sets $\mathcal{D}^j$ (Figure \ref{fig:mcdvcycle}).  The nonlinear operator $N^j$ is constructed in the next subsection, as is the nontrivial \pr{pngs} smoother.
...
\subsection{FE discretization and projected nonlinear Gauss-Seidel} \label{subsec:pngs}  The discretization of SIA VI \eqref{eq:siaVI}, to generate either the single-level form \eqref{eq:siaVIFEfinelevel} or the coarse-level correction form \eqref{eq:siaVIFEFAS}, proceeds essentially as for the classical obstacle problem VI \eqref{eq:obstacleviresidual}.  One difference is that we will approximate the integral in the nonlinear operator $N_\eps$ using quadrature.  Another difference is that the bed elevation $b$ appears in the formula for $N_\eps$, and not just in defining the admissible sets.  In this subsection we first define the operator approximation $N^j$ and then consider a Gauss-Seidel smoother for the VI \eqref{eq:siaVIFEfinelevel} or \eqref{eq:siaVIFEFAS}.

Suppose $w^j$ in $\mathcal{V}^j \subset \mathcal{W}$ is an iterate on the $j$th level, an approximation of the surface elevation $s_\eps$ which solves \eqref{eq:siaVI}, expanded in hat functions $w^j = \sum w_q \psi_q^j$.  (Recall $\mathcal{V}^j$ contains all piecewise-linear functions with zero boundary values.)  Assume $b^j$ interpolates the fine-level bed $b^J$ onto the $j$th level, i.e.~$b^j = \iR b^{j+1}$ for $j=0,\dots,J-1$, again expanded in the basis $b^j = \sum b_q \psi_q^j$.  Also, we give names to the power functions appearing in formula \eqref{eq:siaN} for $N_\eps$:
\begin{equation}
\mu(x) = |x|^{\pp-2} x, \qquad \tau_q(x)  = (x - b_q + \eps)^{\pp+1}, \label{eq:siahelperfcns}
\end{equation}
for $x$ in $\RR$ and $x\ge b_q$, respectively.  Note that $\tau_q$ refers to a pointwise bed elevation value $b_q$.  These functions are continuously-differentiable with nonnegative derivatives $\mu'(x) = (\pp-1) |x|^{\pp-2}$ and $\tau_q'(x) = (\pp+1) (x-b_q + \eps)^{\pp}$, respectively.

Recalling that the support of $\psi_p^j$ is the interval $[x_p^j-h,x_p^j+h]$, the exact value we want is
\begin{equation}
N_\eps(w^j)[\psi_p^j] = \int_{x_p^j-h}^{x_p^j+h} \Gamma (w^j-b+\eps)^{\pp+1} \mu((w^j)') (\psi_p^j)'\,dx.  \label{eq:siaNFEgoal}
\end{equation}
Here $b$ is the original bed elevation function, but to represent $N_\eps$ on the level we must approximate using the interpolated bed $b^j$.  Note that exact integration here is impossible for non-integer $\pp$, and tedious even for $\pp=2$ or the typical value $\pp=4$.  Thus our approximation to \eqref{eq:siaNFEgoal} uses $b^j$, the piecewise-linearity of the functions, and the trapezoid rule on each element:
\begin{align}
...
\end{align}
This defines a map from $w^j$ in $\{v \ge b^j\} \subset \mathcal{V}^j$ to $N^j(w^j)$ in $(\mathcal{V}^j)'$.

Now assume that $\ell^j[\cdot]$ is a linear functional with values $\ell^j[\psi_p^j] = \ell_p$.  (In fact $\ell^j$ is defined in \eqref{eq:siaell}.)  This allows us to define the $j$th-level residual, again in $(\mathcal{V}^j)'$:
\begin{equation}
F^j(w^j) = N^j(w^j) - \ell^j. \label{eq:siaresiduallevel}
\end{equation}

To construct a smoother for VI \eqref{eq:siaVIFEfinelevel} or \eqref{eq:siaVIFEFAS} we follow the derivation of the PGS smoother in the previous section.  Although a minimization-based derivation was possible for the classical obstacle problem, we showed that one-dimensional VI \eqref{eq:pgspointwisevi} also produced the PGS formula \eqref{eq:pgsformula}.
...
\end{align}
is a differentiable function on $[\phi_p - y_p,+\infty)$.  In using this formula it is important to remember that $w_p = g_p+y_p$ and so on.
