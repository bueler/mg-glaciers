...
FIXME new in above is a nonlinear operator $N^{J-1}$ (defined in next subsection) and injection restriction $\hat R:\mathcal{V}^J \to \mathcal{V}^{J-1}$ which preserves admissibility in the original sense: $\hat R:\{v\ge b^J\} \to \{v\ge b^{J-1}\}$.  We rewrite \eqref{eq:siaVIFEFASEARLY}
\begin{equation}
N^{J-1}\left(\hat R(w^J+y^J) + y\right)[v-y] \ge \ell^{J-1}[v-y] \label{eq:siaVIFEFAS}
\end{equation}
for $y$ in $\mathcal{K}^{J-1}$ and all $v$ in $\mathcal{K}^{J-1}$.  The new linear function includes the restricted residual $F^J$ and the $y$-independent term moved from the left side:
   $$\ell^{J-1}[v] = N^{J-1}\left(\hat R(w^J+y^J)\right)[v] - R' F^J[v].$$
