...
As linear operators, $R$ and $P$ may be represented as sparse non-square matrices, and then they are transposes ($P=R^\top$), but using memory to store them this way is unnecessary as they can be applied in $O(m_j)$ operations via \eqref{eq:canonicalrestrictionaction} and \eqref{eq:canonicalprolongationaction}.  Note the two operators differ in their invertibility.  A function $y$ in $\mathcal{V}^{j-1}$ can be exactly recovered from $Py$ because $y(x)$ and $(Py)(x)$ are the same piecewise-linear function, and in fact we only explicitly write $P$ when indicating a change in vector representation.  (Said differently, the one-to-one map $P$ has a left inverse.)  By contrast, $\ell$ in $(\mathcal{V}^j)'$ is not recoverable from $R\ell$; the values $\ell[\psi_q^j]$, the action of $\ell$ on the finer level hats, are averaged onto the coarse level and thus lost.

It turns out that four such \emph{transfer} operators are used in this paper (Table \ref{tab:transfers}), but two of these are introduced in the next section.
...
The following pseudocode uses \pr{pgs-sweep} or \pr{pjacobi-sweep} as a smoother and coarse-level solver.  It takes $F^J$, defined in \eqref{eq:residuallinearlevel}, as an argument, and not the original $F$; it only knows about the fine-level iterate $w^J$ through $F^J$ and $\chi^J$.
...
\pr{mcdl-slash}(F^J,\chi^J,\id{down}=1,\id{coarse}=1)\text{:} \\+
    for $j=J$ downto $j=1$ \\+
      ...
      $y^j = 0$ \\
      $\text{\pr{smoother}}^{\text{\id{down}}}(j,y^j,F^j,\phi^j)$ \qquad\quad \ct{in-place smoothing} \\
...
The following solver adds an outer loop which sets-up the fine-level data $\chi^J$ and $F^J$, calls the slash cycle, and checks for convergence.  Compare \pr{msd-solver} in section \ref{sec:subspace}.
...
These pseudocodes have been implemented in Python.\footnote{At\, \href{https://github.com/bueler/mg-glaciers/}{\texttt{github.com/bueler/mg-glaciers/}} see the \texttt{py/1D/} directory and its \texttt{README.md}.}  We test \pr{mcdl-solver} on the two exact solutions $u$ shown in Figure \ref{fig:icelike}.  Note that the ``ice-like'' case has a discontinuous source $f$, while ``traditional'' has $f=0$, and both have smooth, parabolic obstacles.  Figure \ref{fig:convergence}, showing the numerical error norm $\|u^h-u\|_2$ as a function of mesh spacing $h$, demonstrates convergence for runs with \id{rtol} $=10^{-7}$.  The convergence rate for the traditional case is the best possible; $O(h^2)$ error is expected for the unconstrained problem \cite{Elmanetal2014}.  The ``ice-like'' rate is reduced because of the lower regularity of $f$.
...
\subsection*{Nested iteration and performance results}  In this section we evaluate the performance of \pr{mcdl-solver} by counting iterations, measuring work units (see below), and measuring run time.  However, multilevel methods can often be accelerated using \emph{nested iteration} \cite{Trottenbergetal2001}.  Here the initial iterate on a given mesh comes from prolonging (interpolating) a converged solution iterate from a coarser mesh.  Figure \ref{fig:fcycle} shows the resulting \emph{F-cycle}, constructed by concatenating slash cycles with an additional prolongation action as follows.

\begin{figure}
\input{tikz/fcycle.tex}
\caption{\emph{Left:} A four-level slash cycle with down-smoother (dots), coarse solver (square), and accumulation of corrections back to the finest level (gray line).  \emph{Right:} An F-cycle prepends shallower slash cycles, with solution prolongation $\hat P$ (equation \eqref{eq:solutionprolongation}; double lines) for the initial iterate in the next cycle.}
\label{fig:fcycle}
\end{figure}
...
This review has been written attempting to use the simplest effective notation, but the amount of notation is still substantial, and furthermore a number of new algorithms are discussed.  This Appendix may help the reader in managing the terminology.  Table \ref{tab:notation} shows the notation, arranged alphabetically where possible.  Tables \ref{tab:pseudocodespoisson} and \ref{tab:pseudocodesobstacle} list the pseudocodes with page number.
