...
\pr{mcdl-solver}(J,w^J,F,\varphi^J,\id{rtol}=10^{-4},\id{cyclemax}=100)\text{:} \\+
...
\subsection*{Performance results and nested iterations}  In this section we evaluate the performance of \pr{mcdl-solver} by counting iterations, measuring work units (see below), and measuring run time.

We compare the performance of four solvers:
\begin{itemize}
\item \textsf{V(1,0)}: \pr{mcdl-solver} with default settings including \id{down}$=1$ in the slash cycle
\item \textsf{V(2,0)}: same but with \id{down}$=2$
\item \textsf{F+V(1,0)}: \pr{mcdl-fcycle} with default settings, followed by \textsf{V(1,0)} above
\item \textsf{Fx2+V(1,0)}: same but with two slash cycles in the F-cycle (\id{nicycles}$=2$)
\end{itemize}
We test these solvers on the ``icelike'' problem but with a more generic random bed, like the one shown in Figure \ref{fig:icelikedecomposition}.  In all cases we use the default \pr{pgs-sweeps} smoother and do slash cycles until convergence, i.e.~\eqref{eq:cpconvergencecriterion} with the default \id{rtol} $=10^{-3}$.
...
% for JJ in 6 7 8; do ../obstacle.py -random -cyclemax 100000 -sweepsonly -jfine $JJ; done
% fine level 6 (m=127): using 2926 applications of smoother -> 2926.000 WU
% fine level 7 (m=255): using 11275 applications of smoother -> 11275.000 WU
% fine level 8 (m=511): using 45461 applications of smoother -> 45461.000 WU
