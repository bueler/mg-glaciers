...
\end{equation}
Equation \eqref{eq:plaplacianpoisson} is relatively well-understood because the calculus of variations can be applied to the weak form; see \cite[section 5.3]{Ciarlet2002} and \cite{BarrettLiu1993}.  (The $\pp$-Laplacian obstacle problem has also been studied \cite[for example]{ChoeLewis1991}.)  A unique solution $u$ in $\mathcal{W}$ exists as long as the source $f$ defines a bounded linear functional on $\mathcal{W}$, for which it suffices that $f$ is in $L^\qq[0,L]$ for $\pp^{-1}+\qq^{-1}=1$.  Note that the $\pp>2$ case is \emph{degenerate} \cite{DiBenedetto2012} in the sense that $|u'|^{\pp-2}$ may go to zero.

Returning to the SIA model, let us assume that the obstacle $b$ is in $\mathcal{W}$, in particular that $b(0)=b(L)=0$.  Tentatively we propose to find a solution $s$ from the admissible set
\begin{equation}
\mathcal{K}_b = \left\{v \ge b\right\} \subset \mathcal{W}.  \label{eq:siaK}
\end{equation}
For the derivation of the weak form we assume $s$ is a well-behaved solution to CP \eqref{eq:siacp}, multiply by a test function $v$ from $\mathcal{K}_b$, and integrate by parts.  A nonlinear operator $N:\mathcal{K}_b \to \mathcal{W}'$ appears, namely
\begin{equation}
...
\subsection{Nonlinear MCD cycles} \label{subsec:mcdn}  We now propose to apply a nonlinear form of the MCD method to the steady SIA VI problem \eqref{eq:siaVIresidual}.  The method is a \emph{full approximation storage} (FAS) scheme \cite{Briggsetal2000,Trottenbergetal2001}, a well-known multilevel strategy for nonlinear PDEs, and also the method generalizes the MCDL method of section \ref{sec:obstacle}.  This framework, which avoids the complexity of methods which track the inactive set (compare \cite{Bueler2016,Jouvetetal2013,JouvetGraeser2013}), leads to robust and scalable numerical solutions.

As before, suppose our fine-level FE space is $\mathcal{V}^J \subset \mathcal{W}$, the space of continuous piecewise-linear functions with zero boundary values.  Suppose $b^J$ in $\mathcal{V}^J$ is the fine-level interpolant of the continuum bed $b$, and assume that our given iterate $w^J$ in $\mathcal{V}^J$ is admissible in the sense that $w^J \ge b^J$.  As in section \ref{sec:obstacle} we define the fine-level defect constraint, as a function and a set:
    $$\chi^J = b^J - w^J, \qquad \mathcal{D}^J = \{v\ge \chi^J\} \subset \mathcal{V}^J.$$
For a perturbation $z^J$ in $\mathcal{V}^J$, the perturbed iterate is admissible ($w^J + z^J \ge b^J$) if and only if $z^J$ is in $\mathcal{D}^J$.

Suppose $N^J:\mathcal{D}^J \to (\mathcal{V}^J)'$ is an FE approximation of the nonlinear operator $N$ \eqref{eq:siaN}.  (We define such a $N^J$ in detail in subsection \ref{subsec:pngs} below.)  Let $\ell^J[v] = \ip{a}{v}$.  Our goal is to solve the fine-level FE version of VI \eqref{eq:siaVI} for an admissible perturbation $z^J$ in $\mathcal{D}^J$, that is,
\begin{equation}
N^J(w^J+z^J)[v-z^J] \ge \ell^J[v-z^J] \label{eq:siaVIFEfinelevel}
\end{equation}
for all $v$ in $\mathcal{D}^J$.  Relative to \eqref{eq:siaVI}, note $s^J=w^J+z^J$ is the desired surface elevation result, and that $v - z^J = (w^J+v)-s^J$.

Finite-dimensional nonlinear VI \eqref{eq:siaVIFEfinelevel} suffices when applying a single-level method.  However, to derive a coarse-level correction, and in keeping with the fundamental MCD idea from section \ref{sec:obstacle}, we split $z^J$ into admissible parts on each level.  As in section \ref{sec:obstacle}, we define a defect constraint $\chi^j$ and a (down-)obstacle $\phi^j$ on each level.  For $j=1,\dots,J$ let
    $$\chi^{j-1} = \mR \chi^j, \qquad \phi^j = \chi^j - \chi^{j-1},$$
and also define $\chi^{-1}=0$ so that $\phi^0=\chi^0$.  (Recall $\mR$ denotes the monotone restriction operator \eqref{eq:monotonerestriction}; see Table \ref{tab:transfers}.  Note that the telescoping-sum decomposition \eqref{eq:telescopingdecomposition} now holds.  (Figure \ref{fig:gooddecomposition} serves as a rough visualization, but with $\varphi^J$ replaced by $b^J$.)

Furthermore, for $j=0,\dots,J$ let
    $$\mathcal{D}^j = \{v \ge \chi^j\}, \qquad \mathcal{K}^j = \{v \ge \phi^j\}$$
be the up- and down-admissible subsets of $\mathcal{V}^j$.  Note that $\mathcal{D}^j = \sum_{k=0}^j \mathcal{K}^k$ for any $j$.

Now suppose $y^J$ in $\mathcal{K}^J$ is an admissible perturbation of $w^J$ which \emph{approximately} satisfies
\begin{equation}
N^J(w^J+y^J)[v-y^J] \ge \ell^J[v-y^J] \label{eq:siaVIFEfineK}
\end{equation}
for all $v$ in $\mathcal{K}^J$.  (Because $y^J$ is from a smaller set than $z^J$, even solving \eqref{eq:siaVIFEfineK} exactly does not yield a solution of \eqref{eq:siaVIFEfinelevel}.)  In fact, suppose we have applied a fine-level smoother, so that the residual of \eqref{eq:siaVIFEfineK} is smooth.  (Such a smoother is described in subsection \ref{subsec:pngs}.)  Denote this $J$th-level residual, a linear functional in $(\mathcal{V}^J)'$, as
\begin{equation}
F^J[v] = N^J(w^J+y^J)[v] - \ell^J[v]. \label{eq:siaVIFEresidualfine}
\end{equation}
One effect of the smoother is to make $F^J$ smooth.

Next we seek a coarser-level correction $y$ in $\mathcal{K}^{J-1}$ so that
\begin{equation}
N^J(w^J+y^J+y)[v-y] \ge \ell^J[v-y] \label{eq:siaVIFEcoarseKearlyone}
\end{equation}
for all $v$ in $\mathcal{K}^{J-1}$.  Subtracting $N^J(w^J+y^J)[\cdot]$ from both sides of \eqref{eq:siaVIFEcoarseKearlyone} reveals the residual:
\begin{align}
N^J(w^J+y^J+y)[v-y] - N^J(w^J+y^J)[v-y] &\ge \ell^J[v-y] - N^J(w^J+y^J)[v-y] \label{eq:siaVIFEcoarseKearlytwo} \\
   &= - F^J[v-y]. \notag
\end{align}
Because the residual $F^J$ is smooth, and the difference of inputs on the left, namely $y$, is smooth, inequality \eqref{eq:siaVIFEcoarseKearlytwo} should be well-represented on the $J-1$ level.

Thus we propose the following coarse-level VI:
\begin{equation}
N^{J-1}(\iR(w^J+y^J)+y)[v-y] - N^{J-1}(\iR(w^J+y^J))[v-y] \ge - R F^J[v-y], \label{eq:siaVIFEFASEARLY}
\end{equation}
for $y$ in $\mathcal{K}^{J-1}$ and all $v$ in $\mathcal{K}^{J-1}$.  This inequality applies canonical restriction $R$ (section \ref{sec:subspace} and Table \ref{tab:transfers}), and it uses new symbols $N^{J-1}$ and $\iR$.  The nonlinear operator $N^{J-1}$ will be detailed in subsection \ref{subsec:pngs}.  The needed \emph{injection restriction} $\iR:\mathcal{V}^j \to \mathcal{V}^{j-1}$ simply keeps the values of the solution at the coarse-level nodes and discards the fine-level nodes in between:
\begin{equation}
  (\iR v)_p = v_{2p}, \label{eq:injectionrestriction}
\end{equation}
for $p=1,\dots,m_{j-1}$.  Note that $\iR$ preserves admissibility relative to the interpolated bed elevation data on each level: $v\ge b^j \implies \iR v \ge b^{j-1}$.

Let
   $$\ell^{J-1}[v] = N^{J-1}\left(\iR(w^J+y^J)\right)[v] - R F^J[v].$$
This is the source term of our coarse-level equation, similar to the source term in the original FAS scheme \cite{Trottenbergetal2001}.  Using $\ell^{J-1}$ we may rewrite \eqref{eq:siaVIFEFASEARLY} as
\begin{equation}
