...
Each $k$th-level obstacle $\phi^k = \chi^k - \chi^{k-1}$ also defines a constraint set $\mathcal{K}^k = \{v \ge \phi^k\} \subseteq \mathcal{V}^k$.  In fact, given a fine-mesh iterate $w^K$ and the corresponding defect constraint $\chi^K = \varphi^K - w^K$, we have a subset sum analogous to \eqref{eq:subspacedecomposition}:
\begin{equation}
...
In the ice context (section \ref{sec:sia}) the obstacle $\varphi$ will be the bed elevation and $w^K$ will be the current estimate of the ice surface elevation, and then $-\chi^K$ is the ice thickness on the fine mesh.  Figure \ref{fig:icelikedecomposition} shows the same constraint decomposition as in Figure \ref{fig:gooddecomposition}, but as though it is a decomposition of the ice into layers.  (Compared to Figure \ref{fig:icelike}, note that the obstacle $\phi$ has a more interesting, but still smooth, bed topography.)  Figure \ref{fig:gooddecomposition} plots the decomposition in the correct sense: each defect constraint $\chi^k$ and obstacle $\phi^k$ is piecewise-linear on its mesh level.
...
FIXME state V-cycle for V(1,0); run as \pr{cd-slash}$(K,r^K,\chi^K)$ where $w^K$ is an existing admissible fine-mesh iterate, $\chi^K=\varphi^K-w^K$, and $r^K[\cdot] = \ell^K[\cdot] - a(w^K,\cdot)$
\begin{pseudo*}
\pr{cd-slash}(k,r,\chi)\text{:} \\+
    $w=0$ \\
    if $k=0$ \\+
        $\text{\pr{pgssweep}}^{\text{\id{coarse}}}(0,w,r,\chi)$ \\
        return $w$ \\-
    $\chi^{k-1} = \hat R \chi$ \\
    $\phi^k = \chi - P\chi^{k-1}$ \\
    $\text{\pr{pgssweep}}^{\text{\id{down}}}(k,w,r,\phi^k)$ \\
    $\tilde r[\cdot] = r[\cdot] - a^k(w,\cdot)$ \\
