...
We must specify the data and parameters of the model.  Suppose a \emph{surface mass balance} function $a(x)$, of size $O(1)$ meters per year, and a \emph{bed elevation} (topography) function $b(x)$, with values in meters above sea level, are given.  The data $a(x)$ and $b(x)$ are assumed time-independent.  Let $n>1$ be the (Glen) power, $\rhoi$ the ice density, and $g$ the acceleration of gravity.  Assuming isothermal conditions \cite{GreveBlatter2009}, let $A>0$ be the (constant) ice softness.  In the implementation all quantities are converted to SI units, and the parameter values used below in experiments are from \cite{Huybrechtsetal1996}: $n=3$, $\rhoi=910 \,\text{kg}\,\text{m}^{-3}$, $g=9.81 \,\text{m}\,\text{s}^{-2}$, and $A=10^{-16} s_a^{-1} \,\text{Pa}^n\,\text{s}^{-1}$, where $s_a = 31556926$ is the number of seconds per year.
...
\end{equation}
where $\Gamma = 2 A (\rhoi g)^n / (n+2)$ is a convenience constant.  As explained in section \ref{sec:obstacle}, the true strong form is then a complementarity problem (CP):
\begin{align}
...
CP \eqref{eq:siacp}, which is comparable to \eqref{eq:obstaclecp}, applies on a fixed interval $[0,L]$, a superset of the glaciated area, with $L$ of order $O(1)$ to $O(1000)$ kilometers.  The data $a(x)$, $b(x)$ must be defined on this interval.  Noting that $H=s-b$ is the \emph{ice thickness}, the inequality constraint could state the nonnegativity of thickness, in which case the zero function is the obstacle.  (This is the form found in \cite{Bueler2016,JouvetBueler2012, Jouvetetal2013,JouvetGraeser2013}; see also \cite{Bueler2020}.)  However, the smoothness of $s(x)$, relative to $H(x)$, has consequences for the effectiveness of multilevel calculations, and for this reason we regard $s$ as the solution and $b$ as the obstacle here.

To state the weak form corresponding to \eqref{eq:siacp} we must decide on appropriate function spaces, so we refer to a better-understood nonlinear problem.  Let $\pp=n+1>2$ and consider the $\pp$-\emph{Laplacian Poisson equation}.  The solution and test functions live in the Sobolev space
\begin{equation}
...
\begin{equation}
-(|u'|^{\pp-2} u')' = f, \qquad \int_0^L |u'|^{\pp-2} u' v'\,dx = \int_0^L f v\,dx,  \label{eq:plaplacianpoisson}
\end{equation}
for all $v$ in $\mathcal{W}$.  The weak form \eqref{eq:plaplacianpoisson}, both as a PDE and in its obstacle problem version \cite{ChoeLewis1991}, is relatively well-understood because the calculus of variations can be applied---see \cite[section 5.3]{Ciarlet2002} and \cite{BarrettLiu1993}.  In fact, a unique solution $u$ in $\mathcal{W}$ exists as long as $f$ defines a bounded linear functional on $\mathcal{W}$, for example if $f$ is in $L^\qq[0,L]$ for $\qq$ conjugate to $\pp$, thus $\pp^{-1}+\qq^{-1}=1$.  However, the $\pp>2$ case of \eqref{eq:plaplacianpoisson} is \emph{degenerate} \cite{DiBenedetto2012} in the sense that where $u'\to 0$ the strong-form coefficient of $u''$ goes to zero.

Let us assume that the obstacle $b$ is in $\mathcal{W}$, in particular such that $b(0)=b(L)=0$.  We propose to find a weak form solution $s$ in the admissible set
\begin{equation}
\mathcal{K}_b = \left\{v \ge b\right\} \subset \mathcal{W}.  \label{eq:siaK}
\end{equation}

The weak form itself is derived, as usual, by assuming $s$ is a well-behaved solution to \eqref{eq:siacp}, multiplying by a test function $v$ from $\mathcal{K}_b$, and integrating by parts.  Let
\begin{equation}
...
Whether written in strong form \eqref{eq:siacp} or weak form \eqref{eq:siaVI}, the SIA problem has a double nonlinearity \cite{Calvoetal2002}, of both the porous medium \cite{Evans2010} and $\pp$-Laplacian types.  That is, compared to \eqref{eq:plaplacianpoisson}, the SIA equation does not even have ``uniform ellipticity'' relative to the $\pp$-Laplacian because the thickness may go to zero, i.e.~the coefficient $(s-b)^{\pp+1}$ may degenerate to zero.  It is important for the reader to recognize that the nonlinear form $N(s)[v]$ will degenerate when either the thickness $s-b$ goes to zero or the surface slope $|u'|$ goes to zero.

Defining
\begin{equation}
F(s)[v] = N(s)[v] - \ip{a}{v}, \label{eq:siaF}
\end{equation}
the nonlinear SIA VI takes the form
\begin{equation}
F(s)[v-s] \ge 0 \quad \text{for all } v \text{ in } \mathcal{K}_b, \label{eq:siaVIresidual}
\end{equation}
the same form as the classical obstacle problem VI \eqref{eq:obstacleviresidual} considered in section \ref{sec:obstacle}.  Regarding the intuition behind \eqref{eq:siaVIresidual}, see Figure \ref{fig:cartoonplane}, and the accompanying text, but note that for general data $b(x)$ the $F$ defined in \eqref{eq:siaF} is \emph{not} the gradient of a scalar objective functional \cite{JouvetBueler2012}.

The well-posedness of weak-form problem \eqref{eq:siaVIresidual}, which was first explored by \cite{Calvoetal2002}, is still not resolved in the literature, although large-scale numerical applications of this model are common.  (For example, see the high-resolution Greenland ice sheet model in section 4.2 of \cite{Bueler2016}, or SIA applications as hybrid ice sheet model components \cite[for example]{Winkelmannetal2011}.)  However, a weak-form solution is known to exist \cite{JouvetBueler2012}.  When $b=0$ this solution is unique, but for general bed topography uniqueness has not been established.  Regarding the proof techniques, note that \cite{JouvetBueler2012} writes the problem as a VI for a power of the thickness, i.e.~for $u=(s-b)^{(2n+2)/n}$, and not for $s$.  However, the surface elevation $s$ of a real glacier or ice sheet is relatively smooth compared to the bed elevation $b$,\footnote{This is especially true if crevassing, and other surficial effects like sastrugi, are ignored, as they are when the ice is assumed to be a viscous fluid.} and so we prefer to apply smoothers and transfer operators to $s$ rather than to the ice thickness $s-b$.
...
\subsection*{The PNGS smoother}  The discretization of SIA VI \eqref{eq:siaVIresidual} proceeds essentially as in section \ref{sec:obstacle}, i.e.~for the VI form \eqref{eq:obstacleviresidual} of the classical obstacle problem.  One difference is that we will approximate the integral in \eqref{eq:siaN}, for the nonlinear operator $N(s)[v]$, using quadrature.  Also recall that when we apply an MCD scheme for an obstacle problem, the coarse-level correction involves transformations of the residual and obstacle from their continuum form to a different level-specific form.  On the other hand, the bed elevation $b$ itself appears in the formula for $N(s)[v]$, and not just in defining $\mathcal{K}_b$.

Suppose $w$ in $\mathcal{V}^j$ is an iterate on the $j$th level, an approximation of the surface elevation $s(x)$, where
\begin{equation}
\mathcal{V}^j \subset \mathcal{W}
\end{equation}
is the subspace of piecewise-linear functions with zero boundary values.  The iterate can be written in hat functions,
\begin{equation}
  w(x) = \sum_{q=1}^{m_j} w_q \psi_q^j(x), \label{eq:siatrialfunction}
\end{equation}
and similarly $b^j = \sum b_q \psi_q^j$ interpolates the bed elevation.  Suppose $\phi^j$ is the obstacle on the $j$th level, with expansion $\phi^j = \sum \phi_q \psi_q^j$.  (Note $\phi^j$ is \emph{not} generally the interpolant of the bed $b(x)$, but rather it is a defect obstacle; see below.)  Let
\begin{equation}
  \mathcal{K}^j = \{v \ge \phi^j\} \subset \mathcal{V}^j. \label{eq:siaadmissiblelevel}
\end{equation}
Assume that $\ell^j[\cdot]$ is a linear functional with values $\ell^j[\psi_p^j] = \ell_p$.  (It is computed below as part of the coarse-level correction process.)  Recall that the support of $\psi_p^j$ is the interval $[x_p^j-h,x_p^j+h]$.  Define two helper functions
\begin{equation}
...
We will approximate $N$ in \eqref{eq:siaN} on the $j$th-level, with exact values
\begin{equation}
N(w)[\psi_p^j] = \int_{x_p^j-h}^{x_p^j+h} \Gamma (w-b)^{\pp+1} \mu(w') (\psi_p^j)'\,dx,  \label{eq:siaNlocal}
\end{equation}
using the trapezoid rule and the piecewise-linearity of the hat functions.  In fact, we define the $j$th-level FE residual $F^j(w)[\cdot] \approx N(w)[\cdot] - \ell^j[\cdot]$, a map from $w$ in $\mathcal{K}^j$ to $F^j(w)$ in $(\mathcal{V}^j)'$, as follows:
\begin{align}
F^j(w)[\psi_p^j] &= \Gamma \left(\frac{\tau_{p-1}(w_{p-1}) + \tau_p(w_p)}{2}\right) \mu\left(\frac{w_p-w_{p-1}}{h}\right)\, \frac{+1}{h}\, h \notag \\
    &\qquad\quad  + \Gamma \left(\frac{\tau_p(w_p) + \tau_{p+1}(w_{p+1})}{2}\right) \mu\left(\frac{w_{p+1}-w_p}{h}\right)\, \frac{-1}{h}\, h - \ell_p, \notag \\
    &= \frac{\Gamma}{2h^{\pp-1}} \Big[(\tau_{p-1}(w_{p-1}) + \tau_p(w_p)) \mu(w_p-w_{p-1})  \label{eq:siaresiduallevel} \\
    &\qquad\qquad\quad - (\tau_p(w_p) + \tau_{p+1}(w_{p+1})) \mu(w_{p+1}-w_p)\Big] - \ell_p. \notag
\end{align}
