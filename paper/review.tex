...
Finally we look at the time per degree of freedom.  This quantity should be $O(1)=O(m^0)$ for any optimal method, i.e.~a solver that does $O(m)$ total work to solve the problem, and this is what is seen for the Poisson equation.  Here we only report times on the finer $J=9,\dots,15$ meshes, with a fit to the five finest results giving $O(m^r)$ for $0.05 \le r \le 0.06$, so the mesh dependence is very modest.  The result that our solvers all require about 1 millisecond per degree of freedom depends completely on the details of our Python implementation, and of course the machine on which these computations were run, but this amount of time serves as comparison point relative to the much more expensive computations needed to solve glacier problems (sections \ref{sec:sia} and \ref{sec:stokes}).
...
Mathematical theory allows us to understand the above performance results, at least in part.   Denote the spacing of the fine ($J$th) mesh by $h$.  Let $u^h$ in $\mathcal{V}^h = \mathcal{V}^J$, satisfying $u^h \ge \varphi^J$, be the exact solution of the finite-dimensional VI
\begin{equation}
  F(u^h)[v-u^h] \ge 0 \quad \text{ for all } v \text{ in $\mathcal{V}^h$ such that } v \ge \varphi^J. \label{eq:feobstaclevioriginal}
\end{equation}
This is the direct FE approximation of \eqref{eq:obstacleviresidual}; compare \eqref{eq:feobstacleviresidual} which we solve on each level.  Also recall that for the classical obstacle problem the residual is the gradient of an objective function $I(w)$.
