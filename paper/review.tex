...
\subsection*{Linear geometric multigrid}  Once the smoother has been applied on the finest ($J$) level, so that the error and the residual no longer contain much energy in the high-frequency modes, the smoother should then be applied on the $J-1$ level, but efficiently using $J-1$ level data structures to represent the problem, and this idea should be applied down the mesh-level hierarchy.  By building efficient coarse-level representations we go beyond multilevel subpace corrections and unlock the power of multigrid.
