...
\subsection{Finite elements for a Poisson model problem} \label{subsec:poissonfe}  In this section we will consider how to solve a simple differential equation, namely a linear Poisson-like problem
\begin{equation}
...
\subsection{Multilevel constraint decomposition (MCD)} \label{subsec:mcd}  How do we maintain admissibility throughout a multilevel cycle while not introducing high-frequencies?
...
\subsection{The SIA obstacle problem} \label{subsec:siaproblem}  Now we will consider an obstacle problem, a nonlinear version of the Poisson equation \eqref{eq:poisson}, namely the steady and isothermal shallow-ice approximation (SIA) model for ice surface elevation \cite{Bueleretal2005,Huybrechtsetal1996,vanderVeen2013}.  The weak form of this model is a nonlinear variational inequality (VI), but as usual we state the problem in strong form first.

We must specify the data and parameters of the 1D model.  Suppose we are given a \emph{surface mass balance} function $a(x)$, of size $O(1)$ meters per year, and a \emph{bed elevation} (topography) function $b(x)$ in meters above sea level.  These data $a(x)$ and $b(x)$ are assumed to be time-independent.  Let $n>1$ be the (Glen) power, $\rhoi$ the ice density, and $g$ the acceleration of gravity.  Assuming isothermal conditions \cite{GreveBlatter2009}, let $A>0$ be the (constant) ice softness.  The values used in experiments are from \cite{Huybrechtsetal1996}: $n=3$, $\rhoi=910 \,\text{kg}\,\text{m}^{-3}$, $g=9.81 \,\text{m}\,\text{s}^{-2}$, and $A=10^{-16} \,\text{Pa}^n\,\text{a}^{-1}$.

The SIA equation determines the \emph{ice surface elevation} $s(x)$, in meters above sea level, on the part of the domain where $s>b$:
\begin{equation}
- \left(\Gamma (s-b)^{n+2} |s'|^{n-1} s'\right)' = a,  \label{eq:sia}
\end{equation}
where $\Gamma = 2 A (\rhoi g)^n / (n+2)$ is a convenience constant.

However, as in section \ref{sec:obstacle}, the strong form is actually a complementarity problem (CP):
\begin{align}
s - b &\ge 0, \label{eq:siacp} \\
- \left(\Gamma (s-b)^{n+2} |s'|^{n-1} s'\right)' - a &\ge 0 \notag \\
(s-b) \left[-\left(\Gamma (s-b)^{n+2} |s'|^{n-1} s'\right)' - a\right] &= 0 \notag
\end{align}
This CP, which is comparable to \eqref{eq:obstaclecp}, applies on a fixed interval $[0,L]$, a superset of the glaciated area, with $L$ of order $O(1)$ to $O(1000)$ kilometers, and the data $a(x)$, $b(x)$ must be defined on this entire interval.  Noting that $H=s-b$ is the \emph{ice thickness}, the inequality constraint could be stated as the nonnegativity of thickness, in which case the zero function is the obstacle.  (Indeed, the model is stated in thickness-based CP or VI form in \cite{Bueler2016,JouvetBueler2012, Jouvetetal2013,JouvetGraeser2013}; see also \cite{Bueler2020}.)

To state the weak form corresponding to \eqref{eq:siacp} we must decide on appropriate function spaces, so we refer to a better-understood nonlinear problem.  Let $\pp=n+1>2$ and consider the $\pp$-\emph{Laplacian (Poisson) equation}.  The solution and test functions for this problem live in the Sobolev space of functions with zero boundary values and $\pp$th-power integrable first derivatives:
\begin{equation}
\mathcal{W} = W_0^{1,\pp}[0,L]  \label{eq:defineW}
\end{equation}
For solution $u(x)$ and source $f(x)$, the strong and weak forms of the $\pp$-Laplacian equation are
\begin{equation}
-(|u'|^{\pp-2} u')' = f \qquad \text{and} \qquad \int_0^L |u'|^{\pp-2} u' v'\,dx = \int_0^L f v\,dx \text{ for all $v$ in $\mathcal{W}$.}  \label{eq:plaplacianpoisson}
\end{equation}
The weak form \eqref{eq:plaplacianpoisson}, including in its obstacle problem version \cite{ChoeLewis1991}, is relatively well-understood because the calculus of variations can be applied; see \cite[section 5.3]{Ciarlet2002} and \cite{BarrettLiu1993}.  In fact, a unique solution $u$ in $\mathcal{W}$ exists as long as $f$ defines a bounded linear functional on $\mathcal{W}$.  (It suffices that $f$ is in $L^\qq[0,L]$ for $\pp^{-1}+\qq^{-1}=1$.)  Note that the $\pp>2$ case of \eqref{eq:plaplacianpoisson} is \emph{degenerate} \cite{DiBenedetto2012} in the sense that where $u'\to 0$ the strong-form coefficient of $u''$ goes to zero.

Let us assume that the obstacle $b$ is in $\mathcal{W}$, in particular so that $b(0)=b(L)=0$.  Tentatively we propose to find a solution $s$ to the weak form of \eqref{eq:siacp} from the admissible set
\begin{equation}
\mathcal{K}_b = \left\{v \ge b\right\} \subset \mathcal{W}.  \label{eq:siaK}
\end{equation}
For the derivation we assume $s$ is a well-behaved solution to \eqref{eq:siacp}, multiply by a test function $v$ from $\mathcal{K}_b$, and integrate by parts.  A nonlinear operator $N:\mathcal{K}_b \to \mathcal{W}'$ appears, namely
\begin{equation}
N(s)[v] = \int_0^L \Gamma (s-b)^{\pp+1} |s'|^{\pp-2} s' v'\,dx. \label{eq:siaN}
\end{equation}
Then by application of the CP we derive a nonlinear VI
\begin{equation}
N(s)[v-s] \ge \ip{a}{v-s} \label{eq:siaVI}
\end{equation}
where $\ip{a}{v} = \int_0^L a(x) v(x)\,dx$ is the usual inner product (i.e.~dual pairing \cite{KinderlehrerStampacchia1980}).  We say $s$ in $\mathcal{K}_b$ solves the SIA weak form if inequality \eqref{eq:siaVI} applies for all $v$ in $\mathcal{K}_b$.

Whether written in strong form \eqref{eq:siacp} or weak form \eqref{eq:siaVI}, the SIA problem has a double nonlinearity \cite{Calvoetal2002}.  That is, as a nonlinear elliptic equation it has both porous medium \cite{Evans2010} and $\pp$-Laplacian types. Compared to the $\pp$-Laplacian equation \eqref{eq:plaplacianpoisson}, the SIA equation does not have ``uniform ellipticity'' because the thickness-dependent coefficient $(s-b)^{\pp+1}$ may degenerate to zero.  In summary, the nonlinear form $N(s)[v]$ degenerates when either the thickness $s-b$ goes to zero or the surface slope $|u'|$ goes to zero.

Defining
\begin{equation}
F(s)[v] = N(s)[v] - \ip{a}{v}, \label{eq:siaF}
\end{equation}
the nonlinear VI \eqref{eq:siaVI} takes the form
\begin{equation}
F(s)[v-s] \ge 0 \quad \text{for all } v \text{ in } \mathcal{K}_b, \label{eq:siaVIresidual}
\end{equation}
the same form as the classical obstacle problem VI \eqref{eq:obstacleviresidual} considered in section \ref{sec:obstacle}.  Regarding the intuition behind \eqref{eq:siaVIresidual}, see Figure \ref{fig:cartoonplane} and the accompanying text.  However, for general data $b(x)$ the $F$ defined in \eqref{eq:siaF} is \emph{not} the gradient of a scalar objective functional.

A solution of \eqref{eq:siaVI} or \eqref{eq:siaVIresidual} is known to exist \cite{JouvetBueler2012}, but in the sense that a power of the thickness, $u=(s-b)^{(2n+2)/n}$, and not $s$ itself, is in $\mathcal{W}$.  When $b=0$ the solution is unique, but in general uniqueness has not been established.  On the other hand, the surface elevation $s$ of a real glacier or ice sheet is relatively smooth compared to the bed elevation $b$,\footnote{This is especially true if crevassing, and other surficial effects like sastrugi, are ignored, as they are when the ice is assumed to be a viscous fluid.} and so we prefer to apply smoothers and transfer operators to $s$ rather than to the ice thickness $s-b$.

\begin{figure}
\includegraphics[width=0.65\textwidth]{fixfigs/siadatafigure.pdf}
\caption{\emph{Top:} An exact solution $s(x)$ for a flat-bed ice sheet.  \emph{Bottom:} The corresponding surface mass balance rate $a(x)$.}
\label{fig:siadatafigure}
\end{figure}

For numerical testing we will use the exact solution shown in Figure \ref{fig:siadatafigure}.  (See section 5.6.3 of \cite{GreveBlatter2009} or section 5.3 of \cite{vanderVeen2013} for specific formulas.)  It has a continuous surface mass balance function $a(x)$ which, like the classical obstacle problem solution shown in Figure \ref{fig:icelike}, has negative values towards either end of the interval.  This generates a free boundary, i.e.~an ablation-caused \emph{glacier margin}, but the gradient is singular at the margin.  Indeed, this $s(x)$ is \emph{not} in the space $\mathcal{W}$ because its margin shape goes like $s(x) \sim |x-x_m|^{1/2}$ if $x_m$ is the position of the margin \cite{Bueleretal2005}.  (In the $L=1800$ km case shown, exact margin positions are $x_m=900\pm 750$ km.)  We will return to this important regularity concern below.

A brief historical summary of the SIA problem may be helpful.  The model was first solved by a finite difference scheme in \cite{Mahaffy1976}, and FE solutions are given in \cite{Calvoetal2002,JouvetBueler2012} among other places.  In fact, large-scale numerical applications of this model are common, and for examples see the high-resolution Greenland ice sheet model in section 4.2 of \cite{Bueler2016}, or the SIA applications as hybrid ice sheet model components \cite[for example]{Winkelmannetal2011}.  The idea that the glacier geometry solves an obstacle problem was introduced by \cite{Calvoetal2002} in the time-dependent, flat-bed case.  Existence was extended to non-flat beds by \cite{JouvetBueler2012}.  However, none of these references use multilevel methods.  These were first applied by Jouvet and others \cite{Jouvetetal2013}, with further results in \cite{JouvetGraeser2013}, using a truncated
non-smooth Newton multigrid method \cite{GraeserKornhuber2009} which tracks the inactive set.


\subsection{Nonlinear MCD cycles} \label{subsec:mcdn}  We propose to apply a nonlinear form of the MCD method to the SIA VI problem.  The method is, simultaneously, a \emph{full approximation storage} scheme \cite{Briggsetal2000,Trottenbergetal2001}, that is, a version of a well-known multilevel strategy for nonlinear PDEs, and a generalization the MCDL method of section \ref{sec:obstacle}.  This framework, which is both geometrically clear and relatively simple, should lead to more robust numerical implementations.  The first step is to look at a two-level method.

As before our fine-level FE space is $\mathcal{V}^J \subset \mathcal{W}$, the space of continuous piecewise-linear functions with zero boundary values.  Suppose $b^J$ in $\mathcal{V}^J$ is the fine-level interpolant of the continuum bed $b$, and assume that an iterate $w^J$ in $\mathcal{V}^J$ is admissible in the sense that $w^J \ge b^J$.  As in section \ref{sec:obstacle} we define the fine-level defect constraint and corresponding set as
    $$\chi^J = b^J - w^J, \qquad \mathcal{D}^J = \{v\ge \chi^J\} \subset \mathcal{V}^J.$$
For a perturbation $z^J$ in $\mathcal{V}^J$, the perturbed iterate is admissible ($w^J + z^J \ge b^J$) if and only if $z^J$ is above the defect obstacle ($z^J \ge \chi^J$) or $z^J$ is in $\mathcal{D}^J$.

Let $N^J:\mathcal{D}^J \to (\mathcal{V}^J)'$ be an FE approximation of $N$ defined in \eqref{eq:siaN}, and let $\ell^a[v] = \ip{a}{v}$.  (We will have more to say about $N^J$ below.)  Given the iterate $w^J$, our goal is to solve the fine-level version of VI \eqref{eq:siaVI} for an admissible perturbation $z^J$ in $\mathcal{D}^J$, that is,
\begin{equation}
N^J(w^J+z^J)[v-z^J] \ge \ell_a[v-z^J] \label{eq:siaVIFEfinelevel}
\end{equation}
for all $v$ in $\mathcal{D}^J$.  (Relative to \eqref{eq:siaVI}, note $s^J=w^J+z^J$ is the desired surface elevation result, and that $(w^J+v)-s^J = v - z^J$.)

Finite-dimensional nonlinear VI \eqref{eq:siaVIFEfinelevel} suffices when applying a single-level method.  However, to derive a coarse-level correction, and in keeping with the fundamental MCD idea from section \ref{sec:obstacle}, we split $z^J$ into fine- and coarser-level parts $y^J,y^{J-1}$ so that their sum $z^J=y^J+y^{J-1}$ is admissible, i.e.~in $\mathcal{D}^J$.  Because the splitting must generalize to a full mesh hierarchy anyway, suppose that for $j=1,\dots,J$ we define a defect constraint and (down-)obstacle on each level,
    $$\chi^{j-1} = \mR \chi^j, \qquad \phi^j = \chi^j - \chi^{j-1},$$
including $\chi^{-1}=0$ so $\phi^0=\chi^0$.  (Recall $\mR$ denotes the monotone restriction operator \eqref{eq:monotonerestriction}---see also Table \ref{tab:transfers}---and note that the telescoping-sum decomposition \eqref{eq:telescopingdecomposition} now holds.  Figure \ref{fig:gooddecomposition} again serves as a visualization, but with ``$\varphi^J$'' replaced by the fine-level bed interpolant $b^J$.)  Futhermore, for $j=0,\dots,J$ let
    $$\mathcal{D}^j = \{v \ge \chi^j\}, \qquad \mathcal{K}^j = \{v \ge \phi^j\}$$
be the up- and down-admissible sets, respectively; both are subsets of $\mathcal{V}^j$.  Finally note that $\mathcal{D}^j = \sum_{k=0}^j \mathcal{K}^k$ for all $j$.

Now suppose we have an iterate $y^J$ in $\mathcal{K}^J \subset \mathcal{D}^J$, an admissible perturbation of $w^J$, so that
\begin{equation}
N^J(w^J+y^J)[v-y^J] \ge \ell_a[v-y^J] \label{eq:siaVIFEfineK}
\end{equation}
approximately holds for all $v$ in $\mathcal{K}^J$.  (Because $y^J$ is from a smaller set than $z^J$, even solving \eqref{eq:siaVIFEfineK} exactly does not yield a solution of \eqref{eq:siaVIFEfinelevel}.)  Specifically, supposed we have applied a fine-level smoother (next subsection) so that the CP residual \eqref{eq:cpresidual} for \eqref{eq:siaVIFEfineK} is smooth and that the difference between $w^J+y^J$ and the desired solution $s^J=w^J+z^J$ is smooth.  FIXME THEN WE TRANSFER DOWN

FIXME OLD STUFF  Substituting $s^J=w^J+y^J$ and $v=w^J+\tilde v$ into \eqref{eq:siaVIFE}, and removing the tilde, $y^J$ should satisfy
In fact, let us suppose that the smoother has been applied and that $y^J$ satisfies \eqref{eq:siaVIFEperturbationFINE} to some accuracy.  Now we seek a coarser-level down-admissible perturbation $y$ in $\mathcal{K}^{J-1}$ so that
\begin{equation}
N(w^J+y^J+y)[v-y] \ge \ell_a[v-y] \label{eq:siaVIFEperturbationCOARSE}
\end{equation}
for all $v$ in $\mathcal{K}^{J-1}$.

However, \eqref{eq:siaVIFEperturbationCOARSE} is expressed on the fine mesh, in the sense that the input $w^J+y^J+y$ into $N$ is in $\mathcal{V}^J$, while for multigrid efficiency we need a representation on the coarser level.  So we try to rewrite \eqref{eq:siaVIFEperturbationCOARSE} so that it is accurately represented on the $J-1$ level.  Subtract the quantity $N(w^J+y^J)[v-y]$ from both sides:
\begin{equation}
N(w^J+y^J+y)[v-y] - N(w^J+y^J)[v-y] \ge \ell_a[v-y] - N(w^J+y^J)[v-y]. \label{eq:siaVIFEcoarselevelEARLY}
\end{equation}
We note that the right-hand side is the residual, up to a sign; we define
    $$F^J(y^J)[v] = N(w^J+y^J)[v] - \ell_a[v]$$
in $(\mathcal{V}^J)'$.

Equation \eqref{eq:siaVIFEcoarselevelEARLY} holds as is on the fine level.  However, if the smoother has been applied on the fine level then $w^J+y^J$ gives only smooth residual, and the difference between the inputs into $N$, namely $y$, should be smooth also.  We therefore propose the following coarse-level VI problem which approximates  \eqref{eq:siaVIFEcoarselevelEARLY}:
\begin{equation}
N^{J-1}\left(R^i(w^J+y^J) + y\right)[v-y] \ge \ell^{J-1}[v-y] \label{eq:SIAVIFEcoarselevel}
\end{equation}
for $y$ in $\mathcal{K}^{J-1}$ and all $v$ in $\mathcal{K}^{J-1}$.  Here $R^i:\mathcal{V}^J \to \mathcal{V}^{J-1}$ is injection, which preserves admissibility in the original sense: $R^i:\{v\ge b^J\} \to \{v\ge b^{J-1}\}$.  The new linear function includes the restricted residual from the right side of \eqref{eq:siaVIFEcoarselevelEARLY} plus the $y$-independent term moved from the left side:
   $$\ell^{J-1}[v] = N^{J-1}\left(R^i(w^J+y^J)\right)[v] - R(F^J(y^J))[v].$$
(Here $R$ is canonical restriction.)  Finally
   $$N^{J-1} = FIXME$$
uses $\mR$ to generate a coarse-level thickness coefficient that is better.  That is, on the coarse level the nonlinear operator is slightly different; it is more ``$\pp$-Laplacian elliptic'' in the sense that the thickness-dependent coefficient is nonzero on a coarse-level superset of the fine-level nonzero set.

FIXME much to do


\subsection{The PNGS smoother} \label{subsec:pngs}  The discretization of SIA VI \eqref{eq:siaVIresidual} proceeds essentially as for the VI form \eqref{eq:obstacleviresidual} of the classical obstacle problem.  One difference is that we will approximate the integral in \eqref{eq:siaN}, for the nonlinear operator $N(s)[v]$, using quadrature.  Also recall that when we apply an MCD scheme for an obstacle problem, the coarse-level correction involves transformations of the residual and obstacle from their continuum form to a different level-specific form.  Also, note that the bed elevation $b$ itself appears in the formula for $N(s)[v]$, and not just in defining $\mathcal{K}_b$.

Suppose $w$ in $\mathcal{V}^j \subset \mathcal{W}$ is an iterate on the $j$th level, an approximation of the surface elevation $s(x)$.  (Recall $\mathcal{V}^j$ contains all piecewise-linear functions with zero boundary values.)  The iterate can be written in hat functions,
\begin{equation}
  w(x) = \sum_{q=1}^{m_j} w_q \psi_q^j(x) \label{eq:siatrialfunction}
\end{equation}
Suppose $\phi^j$ is the obstacle on the $j$th level, with expansion $\phi^j = \sum \phi_q \psi_q^j$.  (Note $\phi^j$ is \emph{not} generally the interpolant of the bed $b(x)$, but rather it is a defect obstacle; see below.)  Let
\begin{equation}
  \mathcal{K}^j = \{v \ge \phi^j\} \subset \mathcal{V}^j \subset \mathcal{W}. \label{eq:siaadmissiblelevel}
\end{equation}
Assume that $\ell^j[\cdot]$ is a linear functional with values $\ell^j[\psi_p^j] = \ell_p$.  (It is defined below as part of the coarse-level correction process.)  Also, define two helper functions
\begin{equation}
\mu(x) = |x|^{\pp-2} x, \qquad \tau_p(x)  = (x - b_p)^{\pp+1}, \label{eq:siahelperfcns}
\end{equation}
on $\RR$ and $x\ge b_p$, respectively.  These functions are continuously-differentiable with nonnegative derivatives $\mu'(x) = (\pp-1) |x|^{\pp-2}$ and $\tau_p'(x) = (\pp+1) (x-b_p)^{\pp}$, respectively.

We will approximate the nonlinear operator $N$ \eqref{eq:siaN} on the $j$th-level.  Recalling that the support of $\psi_p^j$ is the interval $[x_p^j-h,x_p^j+h]$, the exact value is
\begin{equation}
N(w)[\psi_p^j] = \int_{x_p^j-h}^{x_p^j+h} \Gamma (w-b)^{\pp+1} \mu(w') (\psi_p^j)'\,dx.  \label{eq:siaNlevelEARLY}
\end{equation}
Our approximation uses the trapezoid rule and the piecewise-linearity of the hat functions:
\begin{align}
N^j(w)[\psi_p^j] &= \Gamma \left(\frac{\tau_{p-1}(w_{p-1}) + \tau_p(w_p)}{2}\right) \mu\left(\frac{w_p-w_{p-1}}{h}\right)\, \frac{+1}{h}\, h \notag \\
...
\end{align}
This is a map from $w$ in $\mathcal{K}^j$ to $N^j(w)[\cdot]$ in $(\mathcal{V}^j)'$.  Now we define the $j$th-level FE residual
\begin{equation}
F^j(w)[\cdot] = N^j(w)[\cdot] - \ell^j[\cdot]. \label{eq:siaresiduallevel}
\end{equation}

To construct a smoother we follow the VI derivation of the PGS smoother in the previous section, wherein one-dimensional VI \eqref{eq:pgspointwisevi} produced formula \eqref{eq:pgsformula}.  Recall that if $w$ is admissible on the $j$th level, i.e.~if $w\ge \phi^j$, then, for piecewise-linear finite elements, a point perturbation $w+c\psi_p^j$ is admissible if and only if $w_p+c \ge \phi_p$, equivalently $c \ge \phi_p - w_p$.  Thus, at the node $x_p^j$, our \emph{projected nonlinear Gauss-Seidel} (PNGS) smoother seeks a scalar $c \ge \phi_p - w_p$ which solves the following one-dimensional VI:
\begin{equation}
  F^j(w+c\psi_p^j)[(w+\tilde c\psi_p^j) - (w+c\psi_p^j)] \ge 0,  \label{eq:pngspointwiseviEARLY}
\end{equation}
for all $\tilde c \ge \phi_p - w_p$.  By linearity, i.e.~because $F^j(w)$ is in $(\mathcal{V}^j)'$, we may write this as
\begin{equation}
  (\tilde c - c) g(c) \ge 0,  \label{eq:pngspointwisevi}
\end{equation}
where
\begin{align}
  g(c) &= F^j(w+c\psi_p^j)[\psi_p^j] = \frac{\Gamma}{2h^{\pp-1}} \Big[(\tau_{p-1}(w_{p-1}) + \tau_p(w_p+c)) \mu(w_p+c-w_{p-1}) \label{eq:pngspointwiseresidual}  \\
       &\hspace{55mm} - (\tau_p(w_p+c) + \tau_{p+1}(w_{p+1})) \mu(w_{p+1}-w_p-c)\Big] - \ell_p \notag
\end{align}
is a differentiable function on $[\phi_p - w_p,+\infty)$.

As was the situation for \eqref{eq:pgspointwisevi}, the solution of VI \eqref{eq:pngspointwisevi} is either the unique value $c$ in the admissible interval $[\phi_p - w_p,+\infty)$ such that $g(c)=0$, or it is the left endpoint of that interval.  In fact, regarding the practical matter of constructing a reliable smoother, it is important that we justify why VI \eqref{eq:pngspointwisevi} is well-posed in this sense.  

FIXME conflict between $[\phi_p - w_p,+\infty)$ and $[b_p - w_p,+\infty)$

FIXME $g(c)$ may not be strictly increasing; its derivative is
\begin{align}
  g'(c) &= \frac{\Gamma}{2h^{\pp-1}} \Big[\tau_p'(w_p+c) \left(\mu(w_p+c-w_{p-1}) -  \mu(w_{p+1}-w_p-c)\right) \label{eq:pngspointwisejacobian} \\
        &\qquad\qquad + (\tau_{p-1}(w_{p-1}) + \tau_p(w_p+c)) \mu'(w_p+c-w_{p-1}) \notag \\
        &\qquad\qquad + (\tau_p(w_p+c) + \tau_{p+1}(w_{p+1})) \mu'(w_{p+1}-w_p-c)\Big] \notag 
\end{align}

FIXME CONSIDER ADDING $\eta_0$ TO THICKNESS

FIXME CONSIDER $w+c\eta_0\psi_p^j$ or $w+ch\psi_p^j$ SO $c$ IS UNITLESS

However, unlike the corresponding problem for the classical obstacle problem, the solution of \eqref{eq:pngspointwisevi} cannot be found exactly.  Instead we set up a Newton iteration to compute $c$.  FIXME

FIXME better patch-type smoother is a $k$-dimensional VI for $k\ge 1$ (but not too big)

FIXME use Bueler profile to verify smoother only

\subsection{Convergence and performance results} \label{subsec:siaperformance}  FIXME
