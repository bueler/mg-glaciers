...
The SIA equation determines the \emph{ice surface elevation} $s(x)$, also in meters above sea level.  Defining a convenience constant $\Gamma = 2 A (\rhoi g)^n / (n+2)$, the equation applies on the part of the domain where $s>b$:
\begin{equation}
...
\end{equation}
(Regarding the powers in \eqref{eq:siacp}, $n+2=\pp+1$ and $n-1=\pp-2$.)  Then by application of the CP we derive our tentative weak form, the nonlinear VI
\begin{equation}
...
The reason this is ``tentative'' requires explanation.  First, whether written in strong form \eqref{eq:siacp} or weak form \eqref{eq:siaVIunreg}, the SIA problem has a well-known ``double nonlinearity'' \cite{Calvoetal2002}.  As a nonlinear elliptic equation, it has both porous medium \cite{Evans2010} and $\pp$-Laplacian types. Compared to \eqref{eq:plaplacianpoisson}, the SIA does not have ``uniform ellipticity'' because the thickness-dependent coefficient $(s-b)^{\pp+1}$ may degenerate to zero.  That is, the nonlinear form $N(s)[v]$ degenerates when either the thickness $s-b$ goes to zero or the surface slope $|u'|$ goes to zero.

Second, consider the flat-bed exact solution shown in Figure \ref{fig:siadatafigure}, which we will also use for numerical testing.  (See section 5.6.3 of \cite{GreveBlatter2009} or section 5.3 of \cite{vanderVeen2013} for specific formulas.)  The continuous surface mass balance function $a(x)$, like the classical obstacle problem solution shown in Figure \ref{fig:icelike}, has negative values towards either end of the interval.  This generates a free boundary, i.e.~an ablation-caused \emph{glacier margin}, where the gradient is singular.  The continuous surface elevation $s(x)$ exactly solves \eqref{eq:sia} on $s>0$, and at the margin the ice flux goes to zero, but $s(x)$ is \emph{not} in $\mathcal{W}$ because its margin shape goes like $s(x) \sim |x-x_m|^{1/2}$ if $x_m$ is the position of the margin \cite{Bueleretal2005}.  (In the $L=1800$ km case shown, exact margin positions are $x_m=900\pm 750$ km.)  This shows VI \eqref{eq:siaVIunreg} cannot be the correct weak form.
...
Let us conjecture a well-posedness theory for \eqref{eq:siaVI}.  For each $\eps>0$ there should be a solution $s_\eps$ in $\mathcal{W}$, also unique in certain circumstances, satisfying \eqref{eq:siaVI}, by a uniform-ellipticity argument relative to the corresponding $\pp$-Laplacian problem.  We do not expect the $\eps\to 0$ limit of $s_\eps$ to be in $\mathcal{W}$.  (The above exact solution suggests $\|s_\eps\|_{\mathcal{W}} \to \infty$ for this case.)  However, we expect that a limit $s(x)$ exists in the sense that $\|s_\eps-s\|_{L^p[0,L]}$ goes to zero, and pointwise.  This conjectured situation is, at least, consistent with all available numerical evidence, but the current paper makes no attempt to prove well-posedness.

In any case, by defining
\begin{equation}
...
\end{equation}
the same form as the classical obstacle problem VI \eqref{eq:obstacleviresidual} considered in section \ref{sec:obstacle}.  Regarding the intuition behind \eqref{eq:siaVIresidual}, see Figure \ref{fig:cartoonplane} and the accompanying text.  However, for general data $b(x)$, note that the $F$ defined in \eqref{eq:siaF} is \emph{not} the gradient of a scalar objective functional.

\subsection{Nonlinear MCD cycles} \label{subsec:mcdn}  We now propose to apply a nonlinear form of the MCD method to the steady SIA VI problem \eqref{eq:siaVI}.  The method is a \emph{full approximation storage} (FAS) scheme, following a well-known multilevel strategy for nonlinear PDEs \cite{Briggsetal2000,Trottenbergetal2001}, and it extends the MCDL method of section \ref{sec:obstacle}.  This framework reduces the implementation complexity of methods which track the inactive set \cite{Bueler2016,Jouvetetal2013,JouvetGraeser2013}, and it leads to robust and scalable numerical solutions.
...
Recall that a perturbed iterate is admissible, $w^J + z^J \ge b^J$, if and only if $z^J$ is in $\mathcal{D}^J$.

As defined in detail in the next subsection \ref{subsec:pngs}, let $N^J$ be an FE approximation of the nonlinear operator $N_\eps$ in \eqref{eq:siaN}.  Let $\ell^J[v] = \ip{a}{v}$.  Our goal is to solve the fine-level FE version of VI \eqref{eq:siaVI} for an admissible perturbation $z^J$ in $\mathcal{D}^J$, that is,
\begin{equation}
N^J(w^J+z^J)[v-z^J] \ge \ell^J[v-z^J] \label{eq:siaVIFEfinelevel}
\end{equation}
for all $v$ in $\mathcal{D}^J$.  Relative to \eqref{eq:siaVI}, note that $s^J=w^J+z^J$ is the desired surface elevation result, and that $v - z^J = (w^J+v)-s^J$.  Finite-dimensional nonlinear VI \eqref{eq:siaVIFEfinelevel} suffices when applying a single-level method.

In keeping with the fundamental MCD idea from section \ref{sec:obstacle}, we derive a coarse-level correction by splitting $z^J$ into admissible parts on each level.  Recalling $\mR$ denotes the monotone restriction operator \eqref{eq:monotonerestriction}, for $j=1,\dots,J$ let
    $$\chi^{j-1} = \mR \chi^j, \qquad \phi^j = \chi^j - \chi^{j-1},$$
and define $\chi^{-1}=0$ so that $\phi^0=\chi^0$.  (See also Table \ref{tab:transfers}.)  The telescoping-sum decomposition \eqref{eq:telescopingdecomposition} now holds.  Furthermore, for $j=0,\dots,J$ let
    $$\mathcal{D}^j = \{v \ge \chi^j\}, \qquad \mathcal{K}^j = \{v \ge \phi^j\}$$
be the up- and down-admissible subsets of $\mathcal{V}^j$, respectively.  Note that $\mathcal{D}^j = \sum_{k=0}^j \mathcal{K}^k$ for any $j$, and that Figure \ref{fig:gooddecomposition} serves as a rough visualization if $\varphi^J$ is replaced by $b^J$.

Now suppose we have an admissible perturbation of $w^J$ which \emph{approximately} satisfies \eqref{eq:siaVIFEfinelevel}.  Specifically, suppose $y^{j+1},\dots,y^J$ in $\mathcal{K}^{j+1},\dots,\mathcal{K}^J$ have been computed, using smoothers like that described in subsection \ref{subsec:pngs}, so that $z^{j+1}=y^{j+1}+\dots+y^J$ approximately solves \eqref{eq:siaVIFEfinelevel}.  We then seek a coarser-level correction $y$ in $\mathcal{K}^j$ so that
\begin{equation}
N^{j+1}(w^J+z^{j+1}+y)[v-y] \ge \ell^{j+1}[v-y] \label{eq:siaVIFEprecoarseone}
\end{equation}
for all $v$ in $\mathcal{K}^j$.  The source $\ell^{j+1}$ is in fact only defined below, but note $\ell^J[v] = \ip{a}{v}$.  Subtracting $N^{j+1}(w^J+z^{j+1})$ from both sides of \eqref{eq:siaVIFEprecoarseone} reveals the residual,
\begin{equation}
...
\end{equation}
The effect of the smoothers was to make $\tilde F^{j+1}$ smooth in the sense that values $\tilde F^{j+1}[\psi_p^{j+1}]$ are relatively small in a CP residual \eqref{eq:cpresidual} sense.

Because \eqref{eq:siaVIFEprecoarsetwo} should be accurately-representable on the $j$th level, we propose the following coarse-correction VI for $y$ in $\mathcal{K}^j$, in preliminary form:
\begin{equation}
N^j(g^j+y)[v-y] - N^j(g^j)[v-y] \ge - R F^{j+1}[v-y], \label{eq:siaVIFEFASearly}
\end{equation}
for all $v$ in $\mathcal{K}^j$.  Note $F^{j+1}[v] = N^{j+1}(g^{j+1}+y^{j+1})[v] - \ell^{j+1}[v]$.

Inequality \eqref{eq:siaVIFEFASearly} applies canonical restriction $R$ on the right, a linear transfer operator defined in section \ref{sec:subspace}, but it also uses a new symbol $g^j$.  They are defined using \emph{injection restriction} $\iR:\mathcal{V}^{j+1} \to \mathcal{V}^j$, which simply keeps the values of the solution at the coarse-level nodes and discards the fine-level nodes in between:
\begin{equation}
  (\iR v)_p = v_{2p}, \label{eq:injectionrestriction}
\end{equation}
for $p=1,\dots,m_j$.  (See Table \ref{tab:transfers}.)  Note that $\iR$ preserves admissibility relative to the interpolated bed elevation data on each level: $v\ge b^{j+1} \implies \iR v \ge b^j$.  Now $g^j$ is the $j$th-level approximation to the solution $s^J$, before the correction $y$ from $\mathcal{K}^j$:
\begin{equation}
...
We can now be completely precise.  Recursively using \eqref{eq:siag} to define $g^j$ from the next-finer correction $y^{j+1}$, let
\begin{equation}
\ell^j[v] = \begin{cases} \ip{a}{v}, & j=J \\
                          N^j(g^j)[v] - R \left(N^{j+1}(g^{j+1}+y^{j+1}) - \ell^{j+1}\right)[v], & j < J.
            \end{cases}  \label{eq:siaell}
\end{equation}
(This is essentially the same source term as in a standard FAS scheme \cite{Trottenbergetal2001}.)  The final form of the coarse-correction VI rewrites \eqref{eq:siaVIFEFASearly} by moving the source term \eqref{eq:siaell} to the right:
\begin{equation}
N^j(g^j+y)[v-y] \ge \ell^j[v-y]. \label{eq:siaVIFEFAS}
\end{equation}

From this statement of the coarse-correction VI we can construct a V-cycle.  Exactly as pictured in Figure \ref{fig:mcdvcycle}, on descent we look for corrections $y^j$ from the down-admissible sets $\mathcal{K}^j$, but going upward we allow corrections $z^j$ from larger up-admissible sets $\mathcal{D}^j$.  We address the \pr{smoother} in the next subsection, as well as how the nonlinear, $j$th-level FE operator is actually constructed.  Compare the following pseudocode to \pr{mcdl-vcycle} on page \pageref{ps:mcdl-vcycle}:
...
In basic usage we will iterate V-cycles until the norm of the CP residual has been reduced by a tolerance factor.  The following pseudocode essentially repeats \pr{mcdl-solver} on page \pageref{ps:mcdl-solver}.
\begin{pseudo*} \label{ps:mcdn-solver}
\pr{mcdn-solver}(J,w^J,N,a,b^J,\id{rtol}=10^{-3},\id{cyclemax}=100)\text{:} \\+
    $r_0=\|\hat\bF(w^J)\|$ \qquad\qquad\qquad\qquad \ct{note $F(z)[\cdot] := N^J(z)[\cdot] - \ip{a}{\cdot}$}\\
    for $s=1,\dots,\id{cyclemax}$ \\+
        $\chi^J = b^J - w^J$ \\
        $\ell^J = \ip{a}{\cdot}$ \\
...
Suppose $w$ in $\mathcal{V}^j \subset \mathcal{W}$ is an iterate on the $j$th level, an approximation of the surface elevation $s(x)$.  (Recall $\mathcal{V}^j$ contains all piecewise-linear functions with zero boundary values.)  The iterate can be written in hat functions, $w = \sum w_q \psi_q^j$.  Suppose $\phi^j$ is the obstacle on the $j$th level, with expansion $\phi^j = \sum \phi_q \psi_q^j$.  (Note $\phi^j$ is \emph{not} generally the interpolant of the bed $b(x)$, but rather it is a defect obstacle; see below.)  Let
\begin{equation}
...
\toprule
\textbf{Name} {\Large$\strut$} & \textbf{Page} & \textbf{Description} \\ \hline
\pr{mcdn-vcycle} & \pageref{ps:mcdn-vcycle} & FAS-type multilevel constraint decomposition (MCD) V-cycle \\
\pr{mcdn-solver} & \pageref{ps:mcdn-solver} & MCD solver; calls \pr{mcdn-vcycle} \\
\pr{pngs} & \pageref{ps:pngs} & projected nonlinear (Newton) GS iteration, a smoother \\ % final \\ required
\bottomrule
