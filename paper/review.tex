...
\pr{pgssweep}(j,w,F,\phi)\text{:} \\+
    ...
    for $p=1,\dots,m_j$ \\+
        ...
        $w \gets w + c \psi_p^j$
\end{pseudo*}
As proven in \cite{BrandtCryer1983}, PGS converges to the solution $y^J$ of problem \eqref{eq:feobstacleviresidual}, but slowly on fine meshes.
...
Now the principle of the MCD method is to \emph{put as much as possible of $\chi^J$ into the coarsest levels.}  To decompose $\chi^J$ we propose a \emph{monotone restriction} operator $\mR$ from $\mathcal{V}^j$ to $\mathcal{V}^{j-1}$.  For $z$ in $\mathcal{V}^j$ one computes $\mR z$ by maximizing the node values of $z$ over the interior of the support of each $j-1$ level hat function.  That is, supposing $z = \sum_q z_q \psi_q^j$, we define
\begin{equation}
...
\end{equation}
(The proof uses the telescoping sum \eqref{eq:telescopingdecomposition}.)  Compare equation \eqref{eq:subspacedecomposition}, wherein the mesh-level subspaces $\mathcal{V}^j$ decompose the whole FE space $\mathcal{V}^j$.

Every admissible modification of the current fine-mesh iterate, i.e.~every $y$ from $\mathcal{V}^J=\mathcal{V}^h$ for which $y\ge \chi^J$, can be built by choosing a function from each of the mesh-level constraint sets $\mathcal{K}^j$.  As with \eqref{eq:subspacedecomposition}, this representation is usually not unique.
