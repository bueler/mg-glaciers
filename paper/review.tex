...
\end{equation}
by using the trapezoid rule.  Thus the $j$th-level residual is computed as follows:
\begin{align}
F^j(w)[\psi_p^j] &= FIXME - \ell_p \label{eq:siaresiduallevel} \\
    &=   \notag
\end{align}

FIXME obstacle $\phi^j$ on that level, $\phi^j = \sum \phi_q \psi_q^j$.  (Note $\phi^j$ is not generally the interpolant of the bed $b(x)$ but instead is a defect obstacle; see below.)
