...
\begin{document}
\title[Geometric multigrid for glacier modeling]{Geometric multigrid for glacier modeling: \\ Core concepts and new techniques}
...
\begin{theorem} \cite[Thm.~3.10]{GraeserKornhuber2009}  Suppose $w^{(s)}$  results from $s$ applications of \pr{msd-slash} or \pr{msd-vcycle} on the fine level, starting with any $w^{(0)}$.  There is $\rho<1$ so that
\begin{equation}
...
\begin{equation}
\varphi(x) = x(1-x), \quad \alpha(x)=0, \quad f(x) = \begin{cases} 8, & 0.2 < x < 0.8, \\
...
\end{equation}
Noting $\nabla I = F$, the VI \eqref{eq:obstacleviresidual} can again be re-stated as follows:
\begin{equation}
  \nabla I(u)[v-u] \ge 0 \quad \text{ for all } v \text{ in } \mathcal{K}. \label{eq:obstaclevigradient}
\end{equation}

Now we can propose a meaning for the VI formulation \eqref{eq:obstaclevigradient}, as it relates to the optimization formulation \eqref{eq:obstaclemin}.  The solution $u$ sits at a location in $\mathcal{K}$, possibly on the boundary of $\mathcal{K}$, where a vector pointing further into the constraint set, namely $v-u$ for $v$ in $\mathcal{K}$, points ``uphill'' on the graph of $I$.  If we think of $\nabla I(u)$ as a vector, instead of a linear functional, we could write \eqref{eq:obstaclevigradient} as $\ip{\nabla I(u)}{v-u} \ge 0$, thus the angle between $\nabla I(u)$ and $v-u$ is at most 90 degrees.  On the other hand, $\mathcal{K} \subset \mathcal{H}$ is an infinite-dimensional set.  To further aid the intuition we propose the following low-dimensional, but global, mental image.  Imagine $\mathcal{K}$ as analogous to the closed first quadrant in the plane, $\mathcal{Q} = \left\{x = (x_1,x_2)\,:\,x_i\ge 0\right\}$, and $I$ as analogous to a smooth, concave-up (i.e.~\emph{coercive} \cite{Evans2010}), objective function $\gamma(x)$ defined on the plane.  The solution of the corresponding VI, say $\hat x$ in $\mathcal{Q}$, may not be a location where $\nabla \gamma$ is zero.  However, $\gamma$ will have as low a value as possible at $\hat x$, even if it is on the boundary of $\mathcal{Q}$.  At $\hat x$ all directions \emph{into} $\mathcal{Q}$ will increase (i.e.~not decrease) the value of $\gamma$, as sketched by contours in Figure \ref{fig:cartoonplane}.  Note that the obstacle problem solution $u$ shown in Figure \ref{fig:icelike} is an infinite-dimensional version of the solution point $\hat x$ shown in the Figure \ref{fig:cartoonplane} cartoon view.  The fact that $u$ in Figure \ref{fig:icelike} coincides with the obstacle $\varphi$ on a part of the domain implies that $u$ is on the boundary of $\mathcal{K}$.
...
The glacier problem in the next section will be formulated both as a CP like \eqref{eq:obstaclecp} and a VI like \eqref{eq:obstaclevi} or \eqref{eq:obstacleviresidual}.  However, for general bed elevation functions (obstacles) the glacier problem has no constrained minimization formulation like \eqref{eq:obstaclemin}, and thus one may not interpret the VI as form \eqref{eq:obstaclevigradient}.  (The essential reason is the lack of a symmetry of the weak form in the general case \cite{JouvetBueler2012}.)  For the glacier problem we possess a map like $F$, from $\mathcal{H}$ to $\mathcal{H}'$, but not a scalar objective like $I$ itself, and thus $F$ is not a gradient.  We will return to this point in section \ref{sec:sia}.
...
Now consider the corresponding FE method, that is, a finite-dimensional version of the same VI.  On the $j$th level we will define an obstacle $\phi^j$ in $\mathcal{V}^j$, and a constraint set $\mathcal{K}^j = \{v \ge \phi^j\} \subset \mathcal{V}^j$, and the FE problem seeks $u^j$ in $\mathcal{K}^j$ so that the finite-dimensional VI holds:
\begin{equation}
  F(u^j)[v-u^j] \ge 0 \quad \text{ for all } v \text{ in } \mathcal{K}^j. \label{eq:feobstacleviresidual}
\end{equation}

As with the linear Poisson problem, we can immediately propose an iterative solution method for solving \eqref{eq:feobstacleviresidual}.  Supposing $w$ in $\mathcal{K}^j$ is an iterate, we modify $w$ at the $p$th node so that \eqref{eq:feobstacleviresidual} holds ``at'' that node.  Note that if $w$ is admissible then $w+c\psi_p^j$ is admissible if and only if $w_p + c \ge \phi_p$ where $\phi_p = \phi^j(x_p^j)$.  (\emph{Warning.}  This equivalence holds for our piecewise-linear elements, but not for higher-order polynomial elements.)  For clarity we first describe the pointwise modification using the constrained minimization form \eqref{eq:obstaclemin}.  We define a convex scalar function from the objective functional,
\begin{equation}
i(b) = I(w+b\psi_p^j),
\end{equation}
where $b$ is a real number, and we seek its minimizer over admissible perturbations,
\begin{equation}
  c = \argmin_{b \ge \phi_p - w_p} \, i(b).
\end{equation}
The minimum of $i(b)$ on the interval $\phi_p - w_p \le b < \infty$ occurs at either the critical point $i'(b)=0$ or at the left end of the interval.  Since $i'(b) = F(w)[\psi_p^j] + b a(\psi_p^j,\psi_p^j)$, we have
\begin{equation}
  c = \max\{-F(w)[\psi_p^j]/a(\psi_p^j,\psi_p^j), \phi_p - w_p\}.  \label{eq:pgsformula}
\end{equation}
We will follow with the update $w \gets w + c\psi_p^j$, and then proceed to the next point.

However, the minimization view is not essential, and we can derive \eqref{eq:pgsformula} using only the VI form.  Given an iterate $w$ in $\mathcal{K}^j$ we seek $c$ such that $w+c\psi_p^j$ is also admissible and so that VI \eqref{eq:obstacleviresidual} holds with all admissible $v=w+\tilde c\psi_p^j$. That is, we find $c\ge \phi_p-w_p$ so that
\begin{equation}
  F(w+c\psi_p^j)[(w+\tilde c\psi_p^j) - (w+c\psi_p^j)] = (\tilde c - c)F(w+c\psi_p^j)[\psi_p^j] \ge 0,  \label{eq:pgspointwisevi}
\end{equation}
for all $\tilde c\ge \phi_p-w_p$.  Inequality \eqref{eq:pgspointwisevi} is a one-dimensional VI, and it implies \eqref{eq:pgsformula}; the argument is elementary calculus.

In conclusion, the following \emph{projected Gauss-Seidel} (PGS) algorithm, a small modification of \pr{gssweep}, implements \eqref{eq:pgsformula}.
\begin{pseudo*}
\pr{pgssweep}(j,w,F,\phi)\text{:} \\+
    for $p=1,\dots,m_j$ \\+
        ...
        $w \gets w + c \psi_p^j$
\end{pseudo*}
If we apply PGS on the fine-level only, instead of in a multilevel method, then we would choose the obstacle $\phi^J$ to be the piecewise-linear interpolant of the continuum obstacle $\varphi$.  In this case, as proven in \cite{BrandtCryer1983}, PGS will converge to the solution $u^J$ of problem \eqref{eq:feobstacleviresidual}, but very slowly on fine meshes.

PGS will serve as our smoother and coarse-level solver, and we will in fact apply only one or two iterations on each level.  However, we are not yet prepared to build a multilevel method for problem \eqref{eq:feobstacleviresidual} because the construction of the obstacles $\phi^j$ and constraint sets $\mathcal{K}^j$ on each mesh level is nontrivial.  In particular, we will \emph{not} choose $\phi^j$ to be the obvious on $j$th-level interpolant of the continuum obstacle $\varphi$.  Instead we will carefully decompose a fine-level constraint across the mesh-level hierarchy.


\subsection*{Multilevel constraint decomposition}  Why is the choice of obstacles in a multilevel scheme nontrivial?  A simple explanation might start from the \pr{gmg-vcycle} algorithm in section \ref{sec:subspace}.  Recall that after solving the coarse-level problem we prolong the correction onto the current level.  In the obstacle problem the updated iterate $w$ would need to be admissible, that is, above the $j$-level obstacle $\phi^j$.  However, as illustrated in Figure \ref{fig:prolongobstacle}, in which the obstacles are interpolants of a continuum obstacle $\varphi$, the prolongation of an iterate which is admissible on the $j-1$ level is \emph{not} necessarily admissible on the $j$ level.  We could replace the iterate by the maximum of the iterate and the obstacle (i.e.~$\max\{w,\phi^j\}$), i.e.~we could enforce admissibility by \emph{truncation}, but this generally introduces high frequencies which may require additional smoother effort to remove.

\begin{figure}
\qquad \includegraphics[width=0.75\textwidth]{genfigs/prolongobstacle.pdf}
\caption{An admissible iterate on the coarse level may not be admissible on the fine level (when prolonged).}
\label{fig:prolongobstacle}
\end{figure}

Our way forward to manage obstacles on multiple mesh levels is the \emph{multilevel constraint decomposition} method of Tai \cite{Tai2003}, in V-cycle form as described by Gr\"aser \& Kornhuber \cite{GraeserKornhuber2009}.  First suppose $w^J$in $\mathcal{V}^J$ is an iterate on the finest grid which is admissible in the obvious sense, namely
\begin{equation}
  w^J \ge \varphi^J \label{eq:fineadmissibleiterate}
\end{equation}
where $\varphi^J$ in $\mathcal{V}^J$ is the interpolant of the continuum obstacle $\varphi$, i.e.~$\varphi^J(x_p^J)=\varphi(x_p^J)$.  We define the \emph{defect constraint} \cite{GraeserKornhuber2009} of $w^J$ to be
\begin{equation}
  \chi^J = \varphi^J - w^J,  \label{eq:defectconstraint}
\end{equation}
which is clearly nonpositive.  The main idea of the defect constraint $\chi^J$ is that if we modify $w^J$ by adding $y$ in $\mathcal{V}^J$ then the result is admissible if and only if $y$ is above the defect constraint:
\begin{equation}
  w^J + y \ge \varphi^J  \qquad \iff \qquad y \ge \chi^J.  \label{eq:defectmeaning}
\end{equation}
The main principle for the constraint decomposition method is that
\begin{quote}
\emph{as much as possible of the fine-level defect constraint $\chi^J$ is in the coarsest levels.}
\end{quote}

To decompose the fine-level defect constraint $\chi^J$ among the mesh levels we define coarser-level functions $\chi^j$ using a new \emph{monotone restriction} operator $\hat R$ from $\mathcal{V}^j$ to $\mathcal{V}^{j-1}$.  For $z$ in $\mathcal{V}^j$ one computes $\hat R z$ by maximizing the node values of $z$ over the interior of the support of each level $j-1$ hat function.  Thus if $z = \sum_q z_q \psi_q^j$ then
\begin{equation}
  \hat R z = \sum_{p=1}^{m_{j-1}} \zeta_p \psi_p^{j-1} \qquad \text{where} \qquad \zeta_p = \max \{z_q \,:\, \psi_p^{j-1}(x_q^j) > 0\},  \label{eq:monotonerestriction}
\end{equation}
so $\hat R z \ge z$.  Note that while canonical restriction \eqref{eq:canonicalrestriction} acts on linear functionals $(\mathcal{V}^j)'$, monotone restriction acts on functions $\mathcal{V}^j$.

Now we define the $j$th-level defect constraint inductively by
\begin{equation}
  \chi^{j-1} = \hat R \chi^j  \label{eq:chik}
\end{equation}
for $j=J$ down to $j=1$.  These defect constraints satisfy two key properties (Figure \ref{fig:gooddecomposition}):
\begin{itemize}
\item $\chi^j$ is in $\mathcal{V}^j$ for $j=0,1,\dots,J$
\item $\chi^j \le \chi^{j-1}$ for $j=1,\dots,J$
\end{itemize}
(Formally one should say ``$\chi^j \le P \chi^{j-1}$'' using the canonical prolongation operator \eqref{eq:canonicalprolongation}, so that we are comparing functions in the same space.)

The \emph{$j$th-level obstacle} is the difference:
\begin{equation}
  \phi^j = \chi^j - \chi^{j-1} \quad \text{ for } j=0,1,\dots,J,  \label{eq:levelobstacle}
\end{equation}
where we also define $\chi^{-1}=0$ so that $\phi^0 = \chi^0$.  Note $\phi^j\le 0$.  The constraint decomposition is now a ``telescoping'' sum which yields the fine-level defect constraint:
\begin{equation}
  \sum_{j=0}^J \phi^j = \chi^0 + (\chi^1 - \chi^0) + (\chi^2 - \chi^1) + \dots + (\chi^J - \chi^{J-1}) = \chi^J.  \label{eq:telescopingdecomposition}
\end{equation}
Note that the sum does \emph{not} give a decomposition of the original obstacle $\varphi^J$, and each term depends on the fine-mesh iterate $w^J$.  An example decomposition is shown in Figure \ref{fig:gooddecomposition}; compare Figure 1 in \cite{GraeserKornhuber2009}.
...
\caption{The constraint decomposition writes a fine-mesh defect obstacle $\chi^J = \varphi^J - w^J$ as a sum of obstacles $\phi^j = \chi^j - \chi^{j-1}$ on each level.}
\label{fig:gooddecomposition}
\end{figure}

From the obstacles $\phi^j$ we define closed, convex \emph{$j$th-level constraint set}
\begin{equation}
\mathcal{K}^j = \left\{v \ge \phi^j\right\} \subseteq \mathcal{V}^j \label{eq:defineKj}
\end{equation}
for all levels $j=0,1,\dots,J$.  Note that $\phi^J$ is not the same as $\varphi^J$, which is the fine-level interpolant of $\varphi$, so $\mathcal{K}^J$ is not an approximation of $\mathcal{K}$.

Recall that the defect constraint on the fine level is $\chi^J = \varphi^J - w^J$.  Let
\begin{equation}
  \mathcal{D}^J = \left\{v \ge \chi^J\right\} \subset \mathcal{V}^J
\end{equation}
be the fine-level \emph{defect set}.  The constraint sets $\mathcal{K}^j$ decompose $\mathcal{D}$ in the same way that the mesh-level subspaces decompose the whole FE space:
\begin{equation}
  \mathcal{D}^J = \mathcal{K}^0 + \mathcal{K}^1 + \dots + \mathcal{K}^J. \label{eq:constraintdecomposition}
\end{equation}
(Compare equation \eqref{eq:subspacedecomposition}.)  The proof uses the telescoping sum \eqref{eq:telescopingdecomposition}.  The meaning of \eqref{eq:constraintdecomposition} is that every admissible modification $v$ of the current fine-mesh iterate, i.e.~every $v$ from the FE space $\mathcal{V}^J=\mathcal{V}^h$ for which $v\ge \chi^J$, can be built, though not uniquely, by choosing functions from each of the mesh-level constraint sets $\mathcal{K}^j = \{v \ge \phi^j\}$.  Equation \eqref{eq:constraintdecomposition} is the heart of the multilevel constraint decomposition method.

In the ice context (section \ref{sec:sia}) the fine-mesh obstacle $\varphi^J$ will be the bed elevation and $w^J$ will be an estimate of the ice surface elevation, so $-\chi^J$ will the ice thickness.  Figure \ref{fig:icelikedecomposition} illustrates the same constraint decomposition as in Figure \ref{fig:gooddecomposition}, but as though it is a decomposition of the ice into layers.  (Compared to Figure \ref{fig:icelike}, note that this bed elevation $\phi^J$ has a bumpy, but smooth, bed topography.)  However, Figure \ref{fig:gooddecomposition} plots the same decomposition in the \emph{correct} sense, i.e.~so that each defect constraint $\chi^j$ and obstacle $\phi^j$ is piecewise-linear on level $j$.  In this sense Figure \ref{fig:icelikedecomposition} is just heuristic.

\begin{figure}
\includegraphics[width=0.7\textwidth]{fixfigs/icedec_defect.pdf}
\caption{A heuristic-only view (Figure \ref{fig:gooddecomposition} shows the correct plot): Multilevel constraint decomposition visualized as a decomposition of the ``ice'' between a fine-mesh iterate $w^J$ and the fine-mesh obstacle $\varphi^J$.}
\label{fig:icelikedecomposition}
\end{figure}

FIXME each level we have the finite-dimensional VI \eqref{eq:feobstacleviresidual}; As in the previous section, $F^J(w)[v]=a(w,v) - \ip{f}{v}$ on the finest level, but on coarser levels we will redefine $F$ before the coarse-level correction
...
\begin{pseudo*}
\pr{mcd-slash}(j,F,\chi)\text{:} \\+
    $y=0$ \\
    if $j=0$ \\+
        $\text{\pr{pgssweep}}^{\text{\id{coarse}}}(0,y,F,\chi)$ \\
        return $y$ \\-
    $\chi^{j-1} = \hat R \chi$ \\
    $\phi^j = \chi - P\chi^{j-1}$ \\
    $\text{\pr{pgssweep}}^{\text{\id{down}}}(j,y,F,\phi^j)$ \\
    $\tilde F[\cdot] = F[\cdot] - a(y,\cdot)$ $< -$ FIXME \\
    $e^{j-1} =$ \pr{mcd-slash}(j-1,R \tilde F,\chi^{j-1}) \\
    return $y + P e^{j-1}$
\end{pseudo*}

FIXME state \pr{mcd-solver}: $w$ is an admissible fine-mesh iterate, $\chi=\varphi^J-w$, $F[\cdot] = a(w,\cdot) - \ip{f}{\cdot}$; apply $w \gets $\pr{mcd-slash}$(J,F,\chi)$; iterate

FIXME PROBABLY WRONG state V(1,1)-cycle; run the same way
\begin{pseudo*}
\pr{mcd-vcycle}(j,r,\chi)\text{:} \\+
    $v=0$ \\
    if $j=0$ \\+
    ...
    $v^{j-1} =$ \pr{mcd-vcycle}(j-1,R \tilde r,\chi^{j-1}) \\
    $v \gets v + P v^{j-1}$ \\
