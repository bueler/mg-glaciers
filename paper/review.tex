...
FIXME however, it is not really an algorithm because of how we represent stuff on $k$th level; if we represent on fine level then only \pr{gsweep}$(K,w,F)$ is fast $O(m_K)$; on other levels $k<K$ we no longer can evaluate $F(w)[\psi_p^k]$ in $O(1)$ time; we need ways of ``moving'' problem to coarser meshes and back
